...
const FORMATION: Position[] = ["GK", "DEF", "DEF", "DEF", "DEF", "MID", "MID", "MID", "MID", "FWD", "FWD"];
    ...
    <div className="space-y-5">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 max-w-2xl text-sm opacity-80">
                Review your own players in full and give the manager a chairman&apos;s selection preference.
              </p>
            </div>
        ...
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              The manager retains final team selection unless a future ownership setting gives you direct control.
            </div>
          </div>
      ...
      <section className="overflow-hidden rounded-2xl border bg-emerald-950 p-4 text-white shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <div className="text-[10px] uppercase tracking-[0.18em] text-white/60">4-4-2 suggestion</div>
            <div className="font-display text-2xl">First XI</div>
          </div>
          <Users className="size-6 text-white/70" />
        </div>
        <Pitch xi={xi} state={state} />
      </section>

      <section className="overflow-hidden rounded-2xl border bg-card shadow-sm">
        <div className="border-b px-4 py-3">
          <div className="font-display text-xl">Wider squad</div>
          <div className="text-xs text-muted-foreground">Exact ability is visible because these are your contracted players.</div>
        </div>
...
function chooseXi(players: FootballPlayer[], preset: Preset, season: number): FootballPlayer[] {
  ...
  return FORMATION.map((position) => {
    const candidates = players
      ...
      .filter((player) => player.primaryPosition === position || player.secondaryPositions.includes(position))
...
function Pitch({ xi, state }: { xi: FootballPlayer[]; state: GameState }) {
  const groups: Array<{ label: string; positions: Position[]; players: FootballPlayer[] }> = [
    { label: "Forwards", positions: ["FWD"], players: xi.filter((p) => p.primaryPosition === "FWD").slice(0, 2) },
    { label: "Midfield", positions: ["MID"], players: xi.filter((p) => p.primaryPosition === "MID").slice(0, 4) },
    { label: "Defence", positions: ["DEF"], players: xi.filter((p) => p.primaryPosition === "DEF").slice(0, 4) },
    { label: "Goalkeeper", positions: ["GK"], players: xi.filter((p) => p.primaryPosition === "GK").slice(0, 1) },
  ];
  const remaining = xi.filter((player) => !groups.some((group) => group.players.some((p) => p.id === player.id)));
    ...
    <div className="grid grid-cols-[1fr_auto] gap-3 px-4 py-3">
      <div className="min-w-0">
        ...
        <div className="mt-0.5 text-xs text-muted-foreground">
          {ageOf(player, state.season)}y · {player.nationality} · Ability {player.currentAbility} · Potential {player.potentialAbility}
        </div>
        <div className="mt-0.5 text-[11px] text-muted-foreground">
          {contract ? `${contract.squadRole} · ${weeksLeftOnContract(state, contract)} weeks left` : "No active contract"}
        </div>
      </div>
...
function PresetButton({ active, onClick, title, sub }: { active: boolean; onClick: () => void; title: string; sub: string }) {
  return (
    ...
    >
      ...
      <div className={cn("text-xs", active ? "text-primary-foreground/70" : "text-muted-foreground")}>{sub}</div>
    </button>
  );
}
