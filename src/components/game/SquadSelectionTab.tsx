...
export function SquadSelectionTab({
  ...
  const choose = (next: Preset) => {
    setPreset(next);
    update((s) => ({ ...s, inboxFlags: { ...s.inboxFlags, "chairman.selection.preset": next, "chairman.selection.ids": chooseXi(userSquad(s), next, s.season, managerFormation(s)).map((player) => player.id).join(",") } }));
  };
  ...
  return (
    <div className="lf-squad-screen flex h-full min-h-0 flex-col gap-3">
      ...
      <div className="contained-scroll touch-pan-y grid min-h-0 flex-1 auto-rows-max gap-3 pr-0.5 lg:grid-cols-[minmax(0,0.9fr)_minmax(22rem,1.1fr)] lg:grid-rows-[auto_auto_minmax(0,1fr)]">
        ...
        {view === "pitch" ? <section className="lf-pitch-card min-h-[29rem] overflow-hidden rounded-xl border bg-emerald-950 p-3 text-white shadow-sm lg:col-start-1"><div className="mb-4 flex items-center justify-between"><div><div className="text-[10px] uppercase tracking-[0.18em] text-white/60">{matchPrep.managerId ? `Manager's XI · ${formation}` : `Caretaker XI · ${formation}`}</div><div className="font-display text-2xl">First XI</div></div><Users className="size-6 text-white/70" /></div><Pitch xi={xi} formation={formation} /></section> : <section className="overflow-hidden rounded-xl border bg-card shadow-sm lg:col-start-1"><div className="border-b px-3 py-2"><div className="font-display text-xl">Squad details & contracts</div><div className="text-xs text-muted-foreground">Important football and contract information in one compact list.</div></div><div className="divide-y">{[...xi, ...bench].map((player) => <CompactPlayerRow key={player.id} state={state} player={player} inXi={selected.has(player.id)} />)}</div></section>}
