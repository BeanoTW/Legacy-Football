...
export function ClubHub({
  ...
  return (
    <div className="lf-home-dashboard flex min-h-0 flex-col gap-3">
      <section className="lf-match-card overflow-hidden rounded-2xl border bg-card shadow-sm">
        <MatchStrip
          state={state}
      ...
      <section className="lf-home-desk grid gap-2 md:grid-cols-[1.15fr_.85fr] md:gap-3">
        <div className="lf-home-panel overflow-hidden rounded-2xl border bg-card shadow-sm">
          <div className="lf-home-panel-heading">
            <div className="flex items-center gap-2">
              <span>Chairman tasks</span>
              {decisionItems.length > 0 && <span className="lf-count-badge">{decisionItems.length}</span>}
            </div>
          ...
          <div className="lf-task-list">
            {topDecisions.length > 0 ? (
              topDecisions.map((item) => (
                <button key={item.id} onClick={() => setTab("inbox")} className="lf-task-row">
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <small className="block truncate">{item.department}</small>
                  </span>
                  <ArrowRight className="size-4 shrink-0 opacity-55" />
                </button>
              ))
              ...
              <div className="lf-task-row is-clear">
                <span className="lf-task-icon"><Mail className="size-4" /></span>
                <span>
                  <strong className="block">No decisions waiting</strong>
                  <small className="block">Club is clear</small>
                </span>
              </div>
            )}
          </div>
        ...
        <button
          ...
          className="lf-news-card overflow-hidden rounded-2xl border bg-card text-left shadow-sm"
        >
          <div className="lf-home-panel-heading">
            <span>Club news</span>
            <span>View all <ArrowRight className="inline size-3.5" /></span>
          </div>
          <div className="lf-news-art" aria-hidden="true" />
          <div className="p-3">
            <strong className="block line-clamp-2 text-sm">
              {latestNews?.subject ?? "Pre-season gets underway"}
            </strong>
            <p className="mt-1 line-clamp-2 text-xs text-muted-foreground">
              {latestNews
                ? `${latestNews.department} · Week ${latestNews.week}`
                : `${state.clubName} begin preparations for the new season.`}
            </p>
          </div>
        </button>
      </section>
      ...
      <section className="lf-management-grid grid grid-cols-2 gap-2 md:grid-cols-3">
        <ActionTile
          ...
          title="Squad"
          value={`${squadSize} players`}
          sub="Selection · contracts"
        />
        <ActionTile
          onClick={() => setTab("recruitment")}
          icon={<Users className="size-5" />}
          title="Transfers"
          value={activeNegotiations > 0 ? `${activeNegotiations} active` : "Market"}
          sub="Scouting · shortlist · deals"
        />
        <ActionTile
        ...
        <ActionTile
          ...
          sub={`${fanbase.toLocaleString()} fans`}
        />
        <ActionTile
          onClick={() => setTab("board")}
          icon={<Target className="size-5" />}
          title="Club vision"
          value="Build for the future"
          sub="Direction · expectations"
        />
      </section>
      ...
      <section className="lf-club-overview overflow-hidden rounded-2xl border bg-card shadow-sm">
        <div className="lf-home-panel-heading">
          ...
          <button onClick={() => setTab("dashboard")}>View more <ArrowRight className="size-3.5" /></button>
        </div>
        <div className="lf-overview-grid">
          <OverviewMetric
            label="Finances"
            value={strategic.health.label}
            detail={`${strategic.health.coverMonths.toFixed(1)} months`}
            className={HEALTH_TONE[strategic.health.state]}
            onClick={() => setTab("cashflow")}
          />
          <OverviewMetric
            label="Reserve"
            value={fmtMoney(strategic.reserve.recommended)}
            detail={
              strategic.reserve.excess > 0
                ? `${fmtMoney(strategic.reserve.excess)} spare`
                : `${fmtMoney(strategic.reserve.deficit)} short`
            }
            onClick={() => setTab("cashflow")}
          />
          <OverviewMetric
            label="Pressure"
            value={`${strategic.pressure.score}/100`}
            detail={strategic.pressure.headline}
            onClick={() => setTab("board")}
          />
          <OverviewMetric
          ...
          <OverviewMetric
            ...
            value={`${managerConf}%`}
            meter={managerConf}
            className={managerConf >= 55 ? "text-amber-600" : "text-rose-600"}
            onClick={() => setTab("staff")}
          />
        </div>
      </section>
...
function MatchStrip({
  ...
  state: GameState;
  nextFixture: GameState["fixtures"][number] | undefined;
  update: (fn: (s: GameState) => GameState) => void;
  onOpenSchedule: () => void;
}) {
  ...
  return (
    <div className="lf-match-inner">
      <div className="lf-match-copy">
        <div className="lf-match-kicker">Next match · Week {state.week}</div>
        <h2>{nextFixture ? "Matchday" : isPreseason ? "Pre-season preparation" : "No fixture this week"}</h2>
        <p>
          {nextFixture
            ? `${nextFixture.home ? "Home" : "Away"} · Saturday · ${state.week <= 6 ? "Friendly" : "League"}`
            : isPreseason
              ? "Friendly · Date TBC · Home"
              : "Use the schedule to review upcoming fixtures."}
        </p>
        <div className="lf-match-actions">
          {matchReady ? (
            <button
              onClick={() => update((current) => startMatchDay(current))}
              className="lf-match-primary"
            >
              <Play className="size-4" /> View match
            </button>
          ) : (
            <button onClick={onOpenSchedule} className="lf-match-primary">
              <Play className="size-4" /> View schedule
            </button>
          )}
          <button onClick={onOpenSchedule} className="lf-match-secondary">
            Match prep
          </button>
        </div>
      </div>

      <div className="lf-match-versus">
        <div className="lf-match-team">
          <div className="lf-team-mark">{initials(homeName)}</div>
          <div className="truncate font-display">{homeName}</div>
        </div>
        <div className="lf-vs">VS</div>
        <div className="lf-match-team">
          <div className={cn("lf-team-mark", !nextFixture && "is-tbc")}>
            {nextFixture ? initials(awayName) : "?"}
          </div>
          <div className="truncate font-display">{awayName}</div>
        </div>
      </div>
    </div>
  );
...
function OverviewMetric({
  ...
  return (
    <button onClick={onClick} className="lf-overview-metric">
      <span>{label}</span>
      <strong className={className}>{value}</strong>
      {detail && <small>{detail}</small>}
      {meter !== undefined && (
        <span className="lf-overview-meter">
...
function LeaguePanel({
  ...
  state: GameState;
  miniLeague: GameState["league"];
  leagueSorted: GameState["league"];
  setTab: (t: Tab) => void;
}) {
  return (
    <section className="flex h-full min-h-0 flex-col overflow-hidden rounded-xl border bg-card shadow-sm">
      ...
      <div className="contained-scroll flex-1">
        <table className="w-full text-[10px] md:text-sm tnum">
          <tbody>
            {miniLeague.map((row) => {
              const pos = leagueSorted.indexOf(row) + 1;
...
function ActionTile({
  ...
  return (
    <button onClick={onClick} className="lf-action-tile">
      <span className="lf-action-icon">{icon}</span>
      <span className="min-w-0 flex-1">
        <span className="lf-action-title">{title}</span>
        <strong className="lf-action-value">{value}</strong>
        {sub && <small className="lf-action-sub">{sub}</small>}
      </span>
