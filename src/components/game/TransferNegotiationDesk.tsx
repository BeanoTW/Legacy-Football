...
function NegotiationRoom({ state, n, act }: { state: GameState; n: TransferNegotiation; act: Act }) {
  ...
  return <div className="space-y-3">
    <section className="overflow-hidden rounded-2xl border bg-card">
      ...
      {incoming && n.competingClubId && n.competingOfferFee !== undefined && <div className="mx-4 mb-4 flex items-start gap-2 rounded-xl border border-amber-500/40 bg-amber-500/10 p-3 text-sm"><TriangleAlert className="mt-0.5 size-4 shrink-0 text-amber-600" /><div><strong>{clubDisplayName(state, n.competingClubId)}</strong> have <strong>{fmtMoneyExact(n.competingOfferFee)}</strong> on the table.<div className="text-xs text-muted-foreground">The seller will not accept less than a live rival bid, and the agent has extra leverage.</div></div></div>}
    ...
    <section className="rounded-2xl border bg-card p-4">
      ...
      {incoming && n.stage === "enquiry" && <NumberField id={`enquiry-fee-${n.id}`} label="Your opening transfer bid" step={feeStep} min={0} value={feeInput ?? feeDefault} onChange={setFeeInput} />}
      {incoming && n.stage === "clubTalks" && <NumberField id={`fee-${n.id}`} label="Your revised transfer fee" step={feeStep} min={n.fee + feeStep} value={feeInput ?? feeDefault} onChange={setFeeInput} />}
      {incoming && n.stage === "playerTalks" && <NumberField id={`wage-${n.id}`} label="Your revised weekly wage" step={wageStep} min={n.proposedWeeklyWage + wageStep} value={wageInput ?? wageDefault} onChange={setWageInput} />}
      <div className="mt-3 flex flex-wrap gap-2">
        {incoming && n.stage === "enquiry" && <Button size="sm" onClick={() => { const fee = Number(feeInput ?? feeDefault); setFeeInput(null); act((s) => submitEnquiryOffer(s, n.id, fee)); }}>Submit opening bid</Button>}
        {incoming && n.stage === "clubTalks" && <Button size="sm" onClick={() => { const fee = Number(feeInput ?? feeDefault); setFeeInput(null); act((s) => improveTransferOffer(s, n.id, fee)); }}>Improve our offer</Button>}
        {incoming && n.stage === "clubTalks" && n.clubCounterFee !== undefined && <Button size="sm" variant="secondary" onClick={() => { setFeeInput(null); act((s) => improveTransferOffer(s, n.id, n.clubCounterFee!)); }}>Accept club counter · {fmtMoneyExact(n.clubCounterFee)}</Button>}
        {incoming && n.stage === "playerTalks" && <Button size="sm" onClick={() => { const wage = Number(wageInput ?? wageDefault); setWageInput(null); act((s) => improvePersonalTerms(s, n.id, wage)); }}>Improve personal terms</Button>}
        {incoming && n.stage === "playerTalks" && n.playerCounterWage !== undefined && <Button size="sm" variant="secondary" onClick={() => { setWageInput(null); act((s) => improvePersonalTerms(s, n.id, n.playerCounterWage!)); }}>Accept agent counter · {fmtMoneyExact(n.playerCounterWage)}/wk</Button>}
