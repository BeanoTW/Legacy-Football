    ...
    <div className="space-y-3 md:space-y-5">
      ...
      <div className="grid grid-cols-2 gap-2 md:gap-3">
        <button
          ...
          className={cn(
            "min-h-16 md:min-h-28 rounded-xl md:rounded-2xl border p-2.5 md:p-4 text-left flex items-center justify-between md:flex-col md:items-start transition-colors",
      ...
      {items.length === 0 ? (
        ...
        <div className="contained-scroll max-h-[52dvh] md:max-h-none space-y-2 md:space-y-3 pr-0.5">
          {items.map((it) => (
            <button
              key={it.id}
              onClick={() => openItem(it)}
              className={cn(
                "w-full min-h-16 md:min-h-24 rounded-xl md:rounded-2xl border bg-card p-3 md:p-4 text-left flex items-center gap-3 md:gap-4 hover:border-primary/40 transition-colors",
                it.status === "awaitingDecision" && "border-amber-500/60 bg-amber-500/5",
              )}
            >
              <div className={cn("size-2.5 md:size-3 rounded-full shrink-0", it.status === "awaitingDecision" ? "bg-amber-500" : it.status === "unread" ? "bg-primary" : "bg-muted-foreground/30")} />
              <div className="flex-1 min-w-0">
                ...
                <div className={cn("text-sm md:text-base mt-0.5 md:mt-1 truncate", (it.status === "unread" || it.status === "awaitingDecision") && "font-semibold")}>{it.subject}</div>
                <div className="text-xs md:text-sm text-muted-foreground truncate">{it.sender}</div>
              </div>
