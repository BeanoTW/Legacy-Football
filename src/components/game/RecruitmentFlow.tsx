...
export function RecruitmentFlow({ state, update }: { state: GameState; update: (fn: (s: GameState) => GameState) => void }) {
  ...
  if (view === "find") {
    ...
    return hasBrief
      ? <ScoutingBrowser state={state} update={update} onBack={() => setView("home")} />
      : <ScoutingBriefBuilder state={state} update={update} onBack={() => setView("home")} />;
  }
  ...
  if (view === "operations") {
    return (
      <div className="flex h-full min-h-0 flex-col gap-3 overflow-hidden">
        <Button className="w-fit shrink-0" variant="ghost" size="sm" onClick={() => setView("home")}><ArrowLeft className="mr-2 size-4" /> Back to transfers</Button>
        <div className="min-h-0 flex-1 overflow-hidden"><RecruitmentOperations state={state} update={update} /></div>
      </div>
    );
  }
    ...
    <OverviewScreen title="Transfers" subtitle="Your football staff bring recruitment options to you. Scout the interesting ones, then decide which deals are worth pursuing." className="grid content-start gap-2 md:gap-3 xl:grid-cols-[minmax(320px,.9fr)_minmax(0,1.6fr)] xl:content-stretch">
      {snap && (
        <section className="lf-transfer-budget flex flex-col justify-center rounded-xl border bg-card p-3 shadow-sm md:p-4">
          ...
          <div className="font-display text-2xl leading-tight md:text-3xl xl:text-4xl">{fmtMoneyExact(snap.budgetRemaining)}</div>
          <div className="mt-2 grid grid-cols-4 gap-1.5 text-center md:mt-3 md:gap-2">
            <MiniStat value={incomingDeals} label="Buying" /><MiniStat value={sales} label="Offers in" /><MiniStat value={activeScouting} label="Scouting" /><MiniStat value={shortlist} label="Shortlist" />
          </div>
        </section>
      )}
...
function MiniStat({ value, label }: { value: number; label: string }) {
  return <div className="rounded-lg bg-muted/50 p-1.5 md:p-2"><div className="font-display text-base md:text-lg">{value}</div><div className="text-[10px] text-muted-foreground md:text-xs">{label}</div></div>;
}
