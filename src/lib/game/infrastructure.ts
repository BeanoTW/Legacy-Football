...
> = {
  Minimal: {
    ...
    decay: 1.75,
    faultRisk: 1.9,
    label: "Only fix what breaks. Costs least, ages fastest.",
  },
  Reduced: {
    cost: 0.7,
    decay: 1.3,
    faultRisk: 1.4,
    label: "Trimmed upkeep. Noticeable long-term wear.",
  },
  Standard: {
    cost: 1.0,
    decay: 1.0,
    faultRisk: 1.0,
    label: "Industry-standard upkeep across the club.",
  },
  Preventative: {
    cost: 1.35,
    decay: 0.72,
    faultRisk: 0.7,
    label: "Planned inspections. Slows decline markedly.",
  },
  Premium: {
    cost: 1.75,
    decay: 0.52,
    faultRisk: 0.45,
    label: "Best-in-class upkeep. Expensive but assets hold.",
  },
};
...
export const ASSET_CONFIG: Record<InfrastructureAssetType, AssetTypeConfig> = {
  ...
  pitch: {
    ...
    decayPerPeriod: 4.6,
    operatingCost: 900,
    maintenanceCost: 1_250,
    maintenanceRequirement: 85,
    maxLevel: 4,
    bucket: "maintenance",
  ...
  sanitary: {
    label: "Toilets & Accessibility",
    decayPerPeriod: 2.8,
    operatingCost: 620,
    maintenanceCost: 380,
    maintenanceRequirement: 70,
    maxLevel: 4,
    bucket: "stadiumOps",
    capacityBands: BAND_FULL,
...
export function ensureInfrastructure(s: GameState): void {
  ...
  if (!existing || !Array.isArray(existing.assets) || existing.assets.length === 0) {
    ...
    const stands: Stand[] =
      Array.isArray(s.stands) && s.stands.length
        ? s.stands
        : [
    ...
    infra.assets.push(
      seedFacility(s, "shop", "shop", "Club Shop", "Main entrance", 2, 0, { floorArea: 180 }),
    );
    infra.assets.push(
      seedFacility(
        s,
    ...
    infra.assets.push(
      seedFacility(
        ...
        "Concourses",
        2,
        Math.round(totalCap * 0.08),
        {},
      ),
    );
    infra.assets.push(
      seedFacility(s, "sanitary", "sanitary", "Toilets & Accessibility", "Concourses", 2, 0, {}),
    );
    infra.assets.push(
      seedFacility(s, "medical", "medical", "Medical Centre", "Training Ground", 2, 0, {}),
    );
    infra.assets.push(
      seedFacility(s, "offices", "offices", "Commercial Offices", "West Stand", 2, 0, {}),
    );
    infra.assets.push(
      seedFacility(s, "fanZone", "fanZone", "Fan Zone", "Stadium forecourt", 1, 0, {}),
    );
  ...
  if (typeof infra.lastDeteriorationPeriod !== "number")
    infra.lastDeteriorationPeriod = periodIndexFor(nowAbs);
...
export const activeProjects = (s: GameState) =>
  projects(s).filter(
    (p) => p.status === "approved" || p.status === "active" || p.status === "delayed",
  );
...
export const criticalAssets = (s: GameState) => assets(s).filter((a) => a.condition < 20);
...
export function facilityModifiers(s: GameState): FacilityModifiers {
  ...
  return {
    ...
    fanHappiness:
      Math.round(
        ((san - 50) * 0.05 + (zone - 50) * 0.035 + (conc - 50) * 0.03 + (standAvg - 50) * 0.04) *
          10,
      ) /
        10 +
      disruptionFans,
    ...
    recruitmentAttraction:
      Math.round(((train - 50) * 0.12 + (med - 50) * 0.06 + (standAvg - 50) * 0.05) * 10) / 10,
...
export function assetCosts(
  ...
  const maintenance = int(
    cfg.maintenanceCost *
      ...
      policy.cost *
      level *
      (1 + a.maintenanceRequirement / 200) *
      (closed ? 0.3 : 1),
  );
...
export function deteriorationFor(s: GameState, a: InfrastructureAsset, period: number): number {
...
export function projectedSeasonDecay(s: GameState, policy: MaintenancePolicy): number {
  ...
  const ratio =
    POLICY_CONFIG[policy].decay /
    POLICY_CONFIG[s.infrastructure?.maintenancePolicy ?? "Standard"].decay;
  const perPeriod = assets(s).length
    ? assets(s).reduce(
        (t, a) => t + deteriorationFor(s, a, periodIndexFor(absoluteWeek(s.season, s.week)) + 1),
        0,
      ) / assets(s).length
    : 0;
...
export function postInfrastructureWeek(s: GameState): void {
  ...
  postEntry(s, {
    ...
    amount: byBucket.trainingOps,
    direction: "expense",
    sourceSystem: "facilities",
    recurring: true,
    dedupeKey: key("trainingOps"),
...
const REPAIR_TYPES: CapitalProjectType[] = [
  "minorRepair",
  "majorRepair",
  "refurbishment",
  "replacement",
  ...
  if (a.type === "stand") {
    ...
    if (has("seatingRefurbishment"))
      out.push({
        ...
        effects: [
          { kind: "metadata", key: "seatingQuality", to: 92 },
          { kind: "condition", add: 10 },
        ],
      });
    if (has("concourseUpgrade"))
      out.push({
        ...
        effects: [
          { kind: "metadata", key: "concourseQuality", to: 90 },
          { kind: "quality", add: 4 },
        ],
      });
    if (has("accessibilityUpgrade"))
      out.push({
    ...
    if (has("hospitalityInstallation"))
      out.push({
        ...
        effects: [
          { kind: "metadata", key: "hospitalityCapacity", add: 180 },
          { kind: "quality", add: 6 },
        ],
      });
    if (has("corporateBoxes"))
      out.push({
        type: "corporateBoxes",
        ...
        effects: [
          { kind: "metadata", key: "hospitalityCapacity", add: 120 },
          { kind: "metadata", key: "commercialSpace", add: 200 },
          { kind: "quality", add: 7 },
        ],
      });
    if (has("retailExpansion"))
      out.push({
        ...
        effects: [
          { kind: "metadata", key: "commercialSpace", add: 260 },
          { kind: "quality", add: 3 },
        ],
      });
    if (has("standRedevelopment"))
      out.push({
...
export function directorPositions(
  ...
  return (s.board?.directors ?? []).map((d) => {
    ...
    const stance: DirectorPosition["stance"] =
      score >= 2 ? "supports" : score <= -1 ? "opposes" : "neutral";
    const note =
      stance === "supports"
...
export function evaluateProject(
  ...
  const committed = activeProjects(s).reduce(
  ...
  if (!assetFree) {
    allowed = false;
    reason = "This asset already has work in progress.";
  } else if (!capacityOk) {
    allowed = false;
    reason = spec.major
...
function applyProjectEffects(s: GameState, p: CapitalProject): void {
  ...
  for (const e of p.effectsOnCompletion) {
    switch (e.kind) {
      case "condition":
        a.condition = clamp(
          e.to != null ? e.to : a.condition + (e.add ?? 0),
          0,
          a.maximumCondition,
        );
        break;
      ...
      case "metadata":
        a.metadata[e.key] =
          e.to != null ? e.to : clamp((a.metadata[e.key] ?? 0) + (e.add ?? 0), 0, 100_000);
        break;
      case "refurbish":
  ...
  for (const p of infra.projects) {
    ...
    for (const pay of p.paymentSchedule) {
      ...
      if (pay.kind === "overrun") {
        p.history.push({
          absoluteWeek: nowAbs,
          note: `Cost overrun of £${pay.amount.toLocaleString("en-GB")} booked.`,
        });
        pushRecord(s, {
          kind: "overrun",
          assetId: a.id,
          assetName: a.name,
          projectId: p.id,
          description: `${p.title} ran over budget`,
          cost: pay.amount,
          capacityBefore: a.capacity,
          capacityAfter: a.capacity,
    ...
    if (blocked) {
      if (p.status !== "delayed") {
        p.history.push({
          absoluteWeek: nowAbs,
          note: "Work paused — the club could not fund this week's payment.",
        });
        pushRecord(s, {
          kind: "delay",
          assetId: a.id,
          assetName: a.name,
          projectId: p.id,
          description: `${p.title} delayed by cash shortage`,
          cost: 0,
          capacityBefore: a.capacity,
          capacityAfter: a.capacity,
          conditionBefore: a.condition,
    ...
    if (p.weeksWorked === p.durationWeeks && p.delayWeeks > 0) {
      p.history.push({
        absoluteWeek: nowAbs,
        note: `Programme running ${p.delayWeeks} week(s) late.`,
      });
      pushRecord(s, {
        kind: "delay",
        assetId: a.id,
        assetName: a.name,
        projectId: p.id,
        description: `${p.title} delayed by ${p.delayWeeks} week(s)`,
        cost: 0,
        capacityBefore: a.capacity,
        capacityAfter: a.capacity,
        conditionBefore: a.condition,
    ...
    if (p.weeksWorked >= total && allPaid) {
      ...
      p.history.push({
        absoluteWeek: nowAbs,
        note: `Completed. Total cost £${p.spentToDate.toLocaleString("en-GB")}.`,
      });
      ...
      pushRecord(s, {
        ...
        assetName: a.name,
        projectId: p.id,
        description: `${p.title} completed`,
        cost: p.spentToDate,
        capacityBefore: capBefore,
        capacityAfter: a.capacity,
        conditionBefore: Math.round(condBefore),
        conditionAfter: Math.round(a.condition),
      });
...
export function cancelProjectInPlace(s: GameState, projectId: string): ProjectActionResult {
  ...
  if (penalty > 0 && int(s.cash) >= penalty) {
    postEntry(s, {
      category: "Facilities",
      subcategory: "Cancellation penalty",
      description: `${p.title} — cancellation penalty`,
      amount: penalty,
      direction: "expense",
      sourceSystem: "facilities",
      linkedEntityId: p.id,
      dedupeKey: `infra-cancel:${p.id}`,
  ...
  p.history.push({
    absoluteWeek: nowAbs,
    note: `Cancelled. £${p.spentToDate.toLocaleString("en-GB")} already spent is not recoverable.`,
  });
  if (a) {
    ...
    pushRecord(s, {
      ...
      assetName: a.name,
      projectId: p.id,
      description: `${p.title} cancelled`,
      cost: penalty,
      capacityBefore: a.capacity,
      capacityAfter: a.capacity,
      conditionBefore: Math.round(a.condition + 4),
      conditionAfter: Math.round(a.condition),
    });
  }
...
export function closeAssetInPlace(s: GameState, assetId: string): ProjectActionResult {
  ...
  a.usableCapacity = 0;
  pushRecord(s, {
    kind: "emergencyClosure",
    assetId: a.id,
    assetName: a.name,
    projectId: null,
    description: `${a.name} closed on safety grounds`,
    cost: 0,
    capacityBefore: a.capacity,
    capacityAfter: 0,
    conditionBefore: Math.round(a.condition),
    conditionAfter: Math.round(a.condition),
  });
  syncLegacyStadium(s);
  return { ok: true };
...
export function reopenAssetInPlace(s: GameState, assetId: string): ProjectActionResult {
  ...
  if (a.status !== "closed") return { ok: true };
  if (a.condition < 20)
    return { ok: false, reason: "Condition is still unsafe — repair it first." };
  ...
  pushRecord(s, {
    ...
    assetName: a.name,
    projectId: null,
    description: `${a.name} reopened to supporters`,
    cost: 0,
    capacityBefore: 0,
    capacityAfter: a.usableCapacity,
    conditionBefore: Math.round(a.condition),
    conditionAfter: Math.round(a.condition),
  });
  syncLegacyStadium(s);
...
export function totalCapitalSpend(s: GameState): number {
  const archived = archivedBucketSum(
    s,
    (b) =>
      b.sourceSystem === "facilities" &&
      b.direction === "expense" &&
      b.subcategory !== "Routine maintenance" &&
      b.subcategory !== "Training ground",
  );
  return (
    archived +
    (s.financeLedger ?? [])
      .filter(
        (e) =>
          e.sourceSystem === "facilities" &&
          e.direction === "expense" &&
          e.subcategory !== "Routine maintenance" &&
          e.subcategory !== "Training ground",
      )
