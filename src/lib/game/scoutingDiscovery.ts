...
import { scoutingPlayerLevelBenchmark, type ScoutingPlayerLevel } from "./chairmanScoutingBrief";
...
interface DetailedCandidate extends CandidateBase { source: "detailed"; player: FootballPlayer; }
interface FringeCandidate extends CandidateBase { source: "fringe"; identity: KnownPlayerSeed; }
...
function fringeCandidates(state: GameState): FringeCandidate[] {
  ...
  for (const club of Object.values(world).sort((a, b) => a.clubId.localeCompare(b.clubId))) for (const position of WORLD_POSITIONS) {
    const projected = projectFringePlayer(state, club, position);
    candidates.push({ id: projected.id, source: "fringe", identity: projected.identity, currentAbility: projected.currentAbility, potentialAbility: projected.potentialAbility, marketValue: projected.marketValue, wageExpectation: projected.wageExpectation, age: projected.age, nationality: projected.identity.nationality, currentClubId: projected.identity.currentClubId, primaryPosition: projected.primaryPosition });
  }
  return candidates;
}
...
function discoveryScore(state: GameState, candidate: DiscoveryCandidate, input: ScoutingBriefInput): number {
  const scoutQuality = scoutingQuality(state);
  const managerRating = state.hiredStaff.find((staff) => staff.role === "Manager")?.rating ?? state.football?.department.recruitmentRating ?? 50;
  const decisionQuality = Math.round(scoutQuality * 0.75 + managerRating * 0.25);
  const targetAbility = 32 + state.reputation * 0.62;
  const legacyAbilityFit = input.minCurrentAbility === undefined ? -Math.abs(candidate.currentAbility - targetAbility) + Math.max(0, candidate.currentAbility - targetAbility) * 0.45 : candidate.currentAbility - input.minCurrentAbility;
  const semanticFit = input.playerLevel ? semanticScoutingFit(candidate, input.playerLevel, scoutingPlayerLevelBenchmark(state, input.playerLevel)) : legacyAbilityFit;
...
export function scoutingBriefDaysRemaining(state: GameState, briefId: string): number { const brief = scoutingBrief(state, briefId); if (!brief || brief.status === "complete") return 0; const dueAtDay = brief.dueAtDay ?? (brief.createdAtAbsoluteWeek * 7 + 4); return Math.max(0, dueAtDay - absoluteDay(state)); }
export function scoutingInitialKnowledge(state: GameState, playerId: string): { days: number; quality: number } { const briefs = (state.football?.scoutingDiscovery?.briefs ?? []).filter((brief) => brief.status === "complete" && brief.candidateIds.includes(playerId)).sort((a, b) => (b.createdAtDay ?? b.createdAtAbsoluteWeek * 7) - (a.createdAtDay ?? a.createdAtAbsoluteWeek * 7)); const brief = briefs[0]; if (!brief) return { days: 0, quality: 50 }; return { days: Math.max(0, Math.min(4, brief.initialKnowledgeDays ?? 2)), quality: Math.max(1, Math.min(100, brief.scoutQuality ?? 50)) }; }
export function scoutingBrief(state: GameState, briefId: string): ScoutingBrief | null { return state.football?.scoutingDiscovery?.briefs.find((brief) => brief.id === briefId) ?? null; }
export function scoutingCandidateSource(state: GameState, briefId: string, playerId: string): ScoutingCandidateSource | null { return scoutingBrief(state, briefId)?.candidateSources?.[playerId] ?? null; }
export function scoutingCandidateProfile(state: GameState, playerId: string): ScoutingCandidateProfile | null { const persistent = state.football?.scoutingDiscovery?.profiles?.[playerId]; if (persistent) return persistent; for (const brief of state.football?.scoutingDiscovery?.briefs ?? []) { const profile = brief.candidateProfiles?.[playerId]; if (profile) return profile; } return null; }
export function discoveredCandidateViews(state: GameState, briefId: string): DiscoveredCandidateView[] { const brief = scoutingBrief(state, briefId); if (!brief || brief.status !== "complete") return []; return brief.candidateIds.flatMap((playerId) => { const detailed = state.football?.players.find((player) => player.id === playerId); const known = knownPlayerIdentity(state, playerId); if (!detailed && !known) return []; const dateOfBirth = detailed?.dateOfBirth ?? known!.dateOfBirth; const source = brief.candidateSources?.[playerId] ?? "detailed"; return [{ playerId, name: detailed ? `${detailed.firstName} ${detailed.lastName}` : `${known!.firstName} ${known!.lastName}`, position: detailed?.primaryPosition ?? known!.primaryPosition, age: BASE_YEAR + state.season - 1 - dateOfBirth.year, clubId: detailed?.currentClubId ?? known!.currentClubId, source, detailed: Boolean(detailed) }]; }); }
export function discoveredPlayerIds(state: GameState): Set<string> { const discovered = new Set<string>(state.football?.scoutingDiscovery?.historicalCandidateIds ?? []); for (const brief of state.football?.scoutingDiscovery?.briefs ?? []) { if (brief.status !== "complete") continue; for (const playerId of brief.candidateIds) discovered.add(playerId); } for (const known of state.football?.playerLifecycle?.knownPlayers ?? []) if (known.reasons.includes("scouted")) discovered.add(known.playerId); return discovered; }
export function isPlayerDiscovered(state: GameState, playerId: string): boolean { const detailed = state.football?.players.find((player) => player.id === playerId); if (isUserClubReference(state, detailed?.currentClubId)) return true; return discoveredPlayerIds(state).has(playerId); }
