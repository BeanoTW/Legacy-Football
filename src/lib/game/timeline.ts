...
export function upcomingTimelineEvents(state: GameState, horizonDays = 42): TimelineEvent[] {
  ...
  for (const fixture of state.fixtures) {
    ...
    events.push({
      ...
      detail: `${fixture.home ? "Home" : "Away"} vs ${clubDisplayName(state, fixture.opponent)}${friendly ? " · Friendly" : ""}`,
    });
  }

  // Pre-season friendlies are generated by the match engine when their week
  // arrives, so they may not exist in state.fixtures yet. Project the known
  // friendly slots now so the chairman can see the match coming in Advance.
  for (const friendlyWeek of FRIENDLY_WEEKS) {
    if (state.fixtures.some((fixture) => fixture.week === friendlyWeek)) continue;
    const absoluteDay = absoluteWeek(state.season, friendlyWeek) * 7 + MATCHDAY_INDEX;
    if (absoluteDay < now || absoluteDay > end) continue;
    events.push({
      id: `friendly:${state.season}:${friendlyWeek}`,
      kind: "fixture",
      absoluteDay,
      week: friendlyWeek,
      day: MATCHDAY_INDEX,
      label: "Friendly",
      detail: "Pre-season friendly",
    });
  }
