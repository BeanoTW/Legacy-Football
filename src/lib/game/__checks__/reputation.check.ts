...
console.log("\n[R2] Promotion raises reputation, relegation lowers it");
{
  ...
  check("every relegated club lost reputation",
...
console.log("\n[R3] Reputation changes stay inside configured limits");
{
  ...
  check("all reputations remain within 0-100",
    Object.values(s.clubReputations).every((v) => v >= REP_MIN && v <= REP_MAX));
  check("delta formula is hard-capped", [
    reputationDelta({ actualFinish: 1, expectedFinish: 20, size: 20, champion: true, runnerUp: false, promoted: true, relegated: false, streak: 3 }),
    reputationDelta({ actualFinish: 20, expectedFinish: 1, size: 20, champion: false, runnerUp: false, promoted: false, relegated: true, streak: -3 }),
...
console.log("\n[R4] Strength is deterministic and derived");
{
  ...
  check("same save => identical strengths",
  ...
  check("different save seed changes strengths",
  ...
  check("strength is not persisted on state",
    !Object.keys(a).some((k) => /strength/i.test(k)));
  ...
  check("no hard tier boundary (best tier-2 beats worst tier-1)",
    Math.max(...t2) > Math.min(...t1));
  const parts = strengthParts(a, a.clubName, 1);
  check("strength decomposes into named parts",
    Math.abs(parts.base + parts.tier + parts.form + parts.movement + parts.variation - parts.total) < 0.02);
}
...
console.log("\n[R5] Stronger clubs win more often over large simulations");
{
  ...
  for (let round = 1; round <= 400; round++) {
    ...
    if (r.homeGoals > r.awayGoals) strongWins++; else if (r.homeGoals < r.awayGoals) weakWins++;
    const r2 = simulateAiFixture(g, 1, round, weak, strong, DIVISION_ONE);
    if (r2.awayGoals > r2.homeGoals) strongWins++; else if (r2.awayGoals < r2.homeGoals) weakWins++;
  }
  check("strongest club beats weakest far more often", strongWins > weakWins * 1.8,
    `${strongWins} vs ${weakWins}`);

  // Table-level: after a season, strength correlates with points.
  const s2 = playSeason(g);
  const finalTable = s2.seasonHistory.find((h) => h.season === 1 && h.leagueId === DIVISION_ONE)!.finalTable;
  ...
  check("pre-season favourites dominate the top half", avgPre < 10.5, `avg pre-season rank ${avgPre}`);
}
...
console.log("\n[R6] Predictions are deterministic and complete");
{
  ...
  check("stored predictions match a fresh calculation",
    JSON.stringify(a.seasonPredictions.filter((p) => p.season === 1)) === JSON.stringify(pa));
  for (const p of pa) {
    check(`${p.leagueId}: predicted champion is the strongest club`,
      p.predictedChampion === p.clubs[0].club && p.clubs[0].rank === 1);
    check(`${p.leagueId}: ranks are 1..n with no gaps`,
...
console.log("\n[R7] Every club receives exactly one expectation");
{
  ...
  const levels: ExpectationLevel[] = ["winLeague", "promotion", "topHalf", "midTable", "avoidRelegation", "survival"];
  check("every club has a valid expectation", all.every((c) => levels.includes(c.expectation)));
  ...
  check("top of tier 2 is asked to push for promotion", expectationFor(1, g.leagues[1]) === "promotion");
}
...
console.log("\n[R8] Auto-resolved user matches are reproducible");
{
  ...
  check("same state => same scoreline",
    ra.goalsFor === rb.goalsFor && ra.goalsAgainst === rb.goalsAgainst,
    `${ra.goalsFor}-${ra.goalsAgainst} vs ${rb.goalsFor}-${rb.goalsAgainst}`);
  check("same attendance and gate", ra.attendance === rb.attendance && ra.gateReceipts === rb.gateReceipts);
  check("same stored match record",
...
console.log("\n[R9] Historical snapshots are immutable");
{
  ...
  check("snapshot carries reputation, strength, expected + actual finish",
    snaps1.every((x) =>
      ...
      x.expectedFinish >= 1 && x.actualFinish >= 1 && typeof x.reputationAfter === "number"));
  ...
  applySeasonIdentity(s3, 1, [{
    leagueId: DIVISION_ONE, tier: 1,
    table: s3.seasonHistory.find((h) => h.season === 1 && h.leagueId === DIVISION_ONE)!.finalTable,
    champion: "", runnerUp: null, promoted: [], relegated: [],
  }]);
...
console.log("\n[R10] League browser data layer");
{
  ...
  for (const lid of [DIVISION_ONE, DIVISION_TWO]) {
    ...
    check(`${lid}: reads live state (all zeroed pre-season)`, t.every((r) => r.p === 0));
    const fx = leagueFixtures(g, lid);
    check(`${lid}: 380 fixtures listed`, fx.length === 380);
    check(`${lid}: nothing played yet`, fx.every((f) => !f.record));
    check(`${lid}: fixtures only from this division`, fx.every((f) => f.league === lid));
  }
  const s2 = playSeason(g);
  for (const lid of [DIVISION_ONE, DIVISION_TWO]) {
    ...
    check(`${lid}: history matches the played fixtures`,
      hist.reduce((a, r) => a + r.p, 0) === 760);
    check(`${lid}: history is sorted by points`,
  ...
  check("browsing history leaves current season untouched",
    JSON.stringify(tableFor(s2, DIVISION_ONE)) === liveBefore);
  check("current season table is empty again after rollover",
...
console.log("\n[R11] Save migration (v4 → v5)");
{
  ...
  check("initClubReputations is deterministic",
    JSON.stringify(initClubReputations(m.leagues, "X")) === JSON.stringify(initClubReputations(m.leagues, "X")));
}
...
console.log("\n[R12] Promoted / relegated clubs evolve rather than jump");
{
  ...
  for (const c of promoted) {
    ...
    check(`promoted ${c} is not instantly dominant in tier ${destination.tier}`,
      clubStrengthFor(s2, c, 2) < avg(destination.clubIds) + 10);
  }
