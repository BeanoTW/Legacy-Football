...
function findCompetingEnquiry(seed: string): { state: GameState; player: FootballPlayer; negotiation: TransferNegotiation } {
  ...
  const candidates = transferMarket(state)
    ...
    .sort((a, b) => a.askingPrice - b.askingPrice || b.player.currentAbility - a.player.currentAbility || a.player.id.localeCompare(b.player.id));

  for (const entry of candidates.slice(0, 100)) {
    const opened = openTransferEnquiryInPlace(state, entry.player.id, "First Team", 25);
    if (opened.ok && opened.negotiation?.competingClubId && opened.negotiation.competingOfferFee !== undefined) {
...
assert.equal(first.negotiation.competingClubId, second.negotiation.competingClubId, "same world should persist the same competing club");
assert.equal(first.negotiation.competingOfferFee, second.negotiation.competingOfferFee, "same world should persist the same competing fee");
...
assert.ok(!sameClubReference(first.state, rivalClubId, first.negotiation.fromClubId), "selling club cannot be its own competing bidder");
assert.ok(buildWorldSimulationPlan(first.state).focusClubIds.includes(rivalClubId), "competing bidder must come from the actively simulated Focus market");
...
const rivalPositionPlayers = rivalSquad.filter((candidate) => candidate.primaryPosition === first.player.primaryPosition);
const rivalPositionCount = rivalPositionPlayers.length;
const weakestRivalAbility = rivalPositionPlayers.length ? Math.min(...rivalPositionPlayers.map((candidate) => candidate.currentAbility)) : 0;
assert.ok(rivalPositionCount < SQUAD_TEMPLATE[first.player.primaryPosition] || first.player.currentAbility - weakestRivalAbility >= 4, "competing bidder must have a genuine shortage or meaningful upgrade need");
assert.ok(clubReputation(first.state, rivalClubId) >= first.player.reputation - 12, "competing bidder must be plausible for the target's reputation");
...
const sellerPolicy = recruitmentTransferFeePolicyForClub(sellerState, sellerNegotiation.fromClubId!);
assert.ok(rivalFee > sellerPolicy.feeStep, "fixture needs room to submit a genuine undercut of the rival bid");
const undercut = rivalFee - sellerPolicy.feeStep;
const undercutResult = submitEnquiryOfferInPlace(sellerState, sellerNegotiation.id, undercut);
assert.ok(undercutResult.ok, undercutResult.reason);
const afterUndercut = negotiationById(sellerState, sellerNegotiation.id)!;
assert.ok(afterUndercut.stage !== "playerTalks" && afterUndercut.stage !== "agreed" && afterUndercut.stage !== "registration" && afterUndercut.stage !== "completed", "seller must not accept less than a live competing bid");
if (afterUndercut.stage === "clubTalks") {
  assert.ok((afterUndercut.clubCounterFee ?? 0) >= rivalFee, "seller counter must respect the standing rival bid");
}
...
function playerCounter(withCompetition: boolean): number {
  ...
  assert.equal(negotiation.stage, "playerTalks", "sub-demand comparison fixture should stay in personal-term talks");
...
assert.ok(competingCounter > normalCounter, `competing interest should strengthen the agent's wage position (£${normalCounter} -> £${competingCounter})`);
console.log(`competing-transfer-bids: passed — ${rivalClubId} bid £${rivalFee.toLocaleString()} for ${first.player.id}`);
