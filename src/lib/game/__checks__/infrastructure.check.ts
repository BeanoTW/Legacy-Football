...
import {
  ...
  periodIndexFor,
  postInfrastructureWeek,
  projectById,
  projectCatalogue,
  projects,
  reopenAssetInPlace,
  riskOutcome,
  runInfrastructureWeek,
  setMaintenancePolicyInPlace,
  specFor,
  stadiumCapacity,
  stadiumUsableCapacity,
  usableCapacityOf,
} from "../infrastructure";
...
function rich(seed = "INFRA_AUDIT", cash = 30_000_000): GameState {
  ...
  postEntry(g, {
    ...
    subcategory: "Benefactor injection",
    description: "Test capital injection",
    amount: cash - Math.round(g.cash),
    direction: "income",
    sourceSystem: "engine.opening",
    dedupeKey: `test-injection:${seed}`,
  });
  return g;
}
...
console.log("\n[A] Asset state");
{
  ...
  check(
    "1. every required asset exists exactly once",
    required.every((r) => ids.filter((i) => i === r).length === 1),
  );
  check("2. asset ids are unique", new Set(ids).size === ids.length);
  check(
    "3. condition is bounded 0-max",
    list.every(
      (a) => a.condition >= 0 && a.condition <= a.maximumCondition && a.maximumCondition <= 100,
    ),
  );
  check(
    "4. usable capacity never exceeds nominal capacity",
  ...
  check(
    "5. closed stands provide zero usable capacity",
...
console.log("\n[B] Deterioration and maintenance");
{
  ...
  check(
    "6. deterioration is deterministic",
  ...
  applyDeterioration(s);
  check(
    "7. deterioration applies once per period",
  ...
  const conds = assets(r)
    .map((x) => x.condition)
    .join(",");
  runInfrastructureWeek(r); // simulated reload replay of the same week
  check(
    "11. reloading cannot duplicate deterioration or maintenance",
    r.cash === snapshotCash &&
      assets(r)
        .map((x) => x.condition)
        .join(",") === conds,
...

{
  ...
  for (const [n, type, min] of [
    ...
    if (!spec) {
      check(n, false, "spec unavailable");
      continue;
    }
    const r = approve(s, target, type);
    if (!r.ok) {
      check(n, false, r.reason);
      continue;
    }
  ...
  check(
    "15. project approval is idempotent (no second project on a busy asset)",
    first.ok && !second.ok && projects(s).length === 1,
    second.reason,
  );
  ...
  const pays = facilityEntries(s).filter((e) => e.linkedEntityId === p.id);
  const keys = pays.map((e) => e.dedupeKey);
  check("16. instalments post exactly once", new Set(keys).size === keys.length && pays.length > 0);
  check(
    "17. progress advances exactly once per week",
    p.weeksWorked <= p.durationWeeks + p.delayWeeks,
  );
  const condAfter = assetById(s, "pitch")!.condition;
  const completionRecords = () =>
    s.infrastructure.history.filter(
      (h) => h.projectId === p.id && h.description.includes("completed"),
    ).length;
  const recBefore = completionRecords();
  runProject(s, 3);
  check(
    "18. completion effects apply once",
    p.effectsApplied === true &&
      completionRecords() === recBefore &&
      assetById(s, "pitch")!.condition <= condAfter,
  );
  const completions = s.infrastructure.history.filter((h) => h.projectId === p.id && h.cost > 0);
  check(
    "19. project history appends once (one costed completion record)",
    completions.length === 1,
  );
  check(
    "20. completed projects never reactivate",
...
console.log("\n[D] Delays and overruns");
{
  ...
  check(
    "22b. different seeds can produce different risk",
    variety.size > 1 &&
      JSON.stringify(r3) === JSON.stringify(riskOutcome("SEED_B", "CP-0001", spec)),
  );
  ...
  for (let i = 0; i < 60 && !found; i++) {
    ...
    if (p.delayWeeks > 0 && p.costOverrun > 0) {
      found = t;
      pid = p.id;
    }
  }
  if (!found) {
    ...
    const overrunEntries = facilityEntries(s0).filter(
      (e) => e.linkedEntityId === pid && e.subcategory === "Project overrun",
    );
    check("26. overrun payment posts exactly once", overrunEntries.length === 1);
    const finalRec = s0.infrastructure.history.filter(
      (h) => h.projectId === pid && h.cost > 0 && h.kind !== "overrun",
    );
    check(
      "27. final history records total cost and final completion date",
      done.status === "completed" &&
        done.completedAtAbsoluteWeek != null &&
        finalRec.length === 1 &&
        finalRec[0].cost === done.spentToDate,
...
console.log("\n[E] Cancellation");
{
  ...
  const longType: CapitalProjectType = specFor(s, standId, "refurbishment")
    ? "refurbishment"
    : "majorRepair";
  ...
  check("28. cancellation is idempotent", c1.ok && c2.ok && s.cash === cashAfterFirst);
  const penalties = facilityEntries(s).filter(
    (e) => e.linkedEntityId === pid && e.subcategory === "Cancellation penalty",
  );
  check("29. cancellation penalty posts once", penalties.length === 1);
  check(
    "29b. penalty is 15% of the unpaid commitment",
    penalties.length === 1 &&
      penalties[0].amount > 0 &&
      Math.abs(penalties[0].amount - (cashBefore - cashAfterFirst)) < 2 &&
      CANCELLATION_PENALTY_PCT === 0.15,
  );
  check(
    "30. spent funds are not refunded",
    s.cash <= cashBefore && projectById(s, pid)!.spentToDate >= spentBefore,
  );
  runProject(s, 20);
  const p = projectById(s, pid)!;
  check(
    "31. cancelled projects cannot complete",
    p.status === "cancelled" && p.effectsApplied !== true,
  );
  const historyBefore = JSON.stringify(s.infrastructure.history);
  runProject(s, 4);
  check(
    "32. cancellation history is immutable",
    JSON.stringify(s.infrastructure.history.slice(0, s.infrastructure.history.length)).startsWith(
      historyBefore.slice(0, historyBefore.length - 1),
    ),
  );
}
...
console.log("\n[F] Director positions");
{
  ...
  broke.cash = 1_000;
  const standSpec =
    projectCatalogue(broke, assets(broke).find((x) => x.type === "stand")!.id).find(
      (x) => x.major,
    ) ?? specFor(broke, "pitch", "majorRepair")!;
  ...
  check(
    "34. Finance and Supporters' directors can disagree",
    !!fin && !!sup && fin.stance !== sup.stance,
    `${fin?.stance} vs ${sup?.stance}`,
  );

  const sporting = directorPositions(
    s,
    "training",
    specFor(s, "training", "facilityUpgrade") ?? specFor(s, "training", "majorRepair")!,
  );
  ...
  check(
    "35. Football Director values sporting projects",
    !!football &&
      football.stance === "supports" &&
  ...
  check("36. Commercial Director values commercial projects", !!comm && comm.stance === "supports");
  ...
  check(
    "37. director positions do not alter confidence on preview",
...
console.log("\n[G] Infrastructure Inbox");
{
  ...
  s = runWeeklyGenerators(s);
  check(
    "38. deterioration warnings generate once",
  ...
  c = runWeeklyGenerators(c);
  check(
    "39. critical warnings generate once",
  ...
  pr = runWeeklyGenerators(pr);
  check(
    "40. project proposals generate once",
  ...
  if (prop.length === 1) {
    ...
    check(
      "48. Inbox actions call canonical infrastructure functions",
      projects(after).length === before + 1 &&
        assetById(after, projects(after)[projects(after).length - 1].assetId)!.activeProjectId !==
          null,
    );
  } else {
  ...
  cx = runWeeklyGenerators(cx);
  check(
    "45. cancellation messages generate once",
    cancelMsgs.length === 1 &&
  ...
  ro = runWeeklyGenerators(ro);
  check(
    "46. reopening messages generate once",
    reopened.length === 1 && ro.inbox.filter((i) => i.subject.startsWith("Reopened")).length === 1,
  );

  // Informational items carry no effects at all.
  const informational = [...w.inbox, ...ro.inbox, ...s.inbox].filter(
    (i) =>
      i.generatorId === "facilities-works-update" ||
      i.generatorId === "facilities-condition-warning",
  );
  check(
    "47. informational messages do not reapply money or effects",
    informational.length > 0 && informational.every((i) => (i.choices?.length ?? 0) === 0),
  );

  check(
    "49b. all facilities generators are registered",
    [
      "facilities-condition-warning",
      "facilities-critical-warning",
      "facilities-project-proposal",
      "facilities-works-update",
    ].every(isKnownGeneratorId),
  );
}
...
console.log("\n[H] Cross-system modifiers");
{
  ...
  check(
    "52. Staff market reads staffAttraction",
    tGood.wageDemand < tBad.wageDemand,
    `${tGood.wageDemand} vs ${tBad.wageDemand}`,
  );
  const impossible = { ...(staff as object), reputation: 99 } as never;
  check(
    "52b. infrastructure does not guarantee acceptance",
    staffJoinTerms(40, impossible, facilityModifiers(recGood).staffAttraction).willing === false,
  );

  const sportingSpec =
    specFor(recBad, "training", "facilityUpgrade") ?? specFor(recBad, "training", "majorRepair")!;
  const fdBad = directorPositions(recBad, "training", sportingSpec).find(
    (p) => p.role === "Football Director",
  )!;
  check(
    "53. Football/Board systems read sportingQuality",
  ...
  const base = rich();
  check(
    "54. same input state produces identical derived modifiers",
  ...
  check(
    "55. UI browsing does not alter state or modifier values",
...
console.log("\n[I] Finance and migration");
{
  ...
  check(
    "56. all infrastructure spending uses postEntry",
...
console.log("\n[J] Static audit");
{
  ...
  check(
    "J1. no Math.random()/Date.now() in simulation modules",
    offenders.length === 0,
    offenders.join(", "),
  );
  check(
    "J2. commercial does not duplicate facility maths",
  ...
  check(
    "J5. inbox never writes infrastructure state directly",
    !/s\.infrastructure\.(assets|projects)\s*\.\s*(push|splice)/.test(src("inbox.ts")),
  );
  check(
    "J6. no cancellation refunds",
