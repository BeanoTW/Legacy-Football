...
assert(candidates.length > 0, "expected at least one contracted Level 7 target affordable on both fee and wage headroom");
...
assert(canAuthoriseWage(state, target.wageDemand).allowed, "selected contracted target must fit the opening wage ceiling at his quoted demand");

const opened = openTransferNegotiationInPlace(state, target.player.id, Math.round(canonicalAsk * 1.2));
...
for (let round = 0; round <= MAX_NEGOTIATION_ROUNDS + 1 && negotiation.stage !== "agreed"; round++) {
  if (negotiation.stage === "clubTalks") {
    const nextFee = Math.max(negotiation.fee + 1, negotiation.clubCounterFee ?? Math.round(canonicalAsk * 1.35));
    ...
    const requestedWage = Math.max(negotiation.proposedWeeklyWage + 1, negotiation.playerCounterWage ?? Math.round(negotiation.proposedWeeklyWage * 1.3));
...
assert(negotiationById(state, negotiation.id)?.stage === "registration", "Level 7 deal should persist registration before completion");
