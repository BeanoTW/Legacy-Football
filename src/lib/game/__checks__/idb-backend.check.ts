...
import { createIdbRecordStore, indexedDbAvailable, DB_NAME, STORE_NAME } from "../storage/idbBackend";
...
check("indexedDB is detected as available", indexedDbAvailable());
{
  ...
  await records.putAll([{ key: "primary:core", value: "a" }, { key: "primary:manifest", value: "b" }]);
  const got = await records.get(["primary:core", "primary:manifest", "primary:missing"]);
  check("putAll + get round-trip", got["primary:core"] === "a" && got["primary:manifest"] === "b" && got["primary:missing"] === undefined);
  await records.putAll([{ key: "other:core", value: "c" }]);
  check("prefix listing is scoped", (await records.keys("primary:")).sort().join(",") === "primary:core,primary:manifest");
  await records.deletePrefix("primary:");
  check("deletePrefix removes only the matching slot",
  ...
  check("database identity is stable", DB_NAME === "football-club-owner" && STORE_NAME === "records");
}
...
console.log("\n[R2] SaveStore on real IndexedDB, with a legacy localStorage save");
{
  ...
  const backend = {
    ...
    setItem: (k: string, v: string) => { legacyMap.set(k, v); },
    removeItem: (k: string) => { legacyMap.delete(k); },
  };
  const s = fresh();
  ...
  check("legacy save migrates into real IndexedDB",
    migrated.state !== null && stateHash(migrated.state!) === stateHash(s) &&
    migrated.diagnostics.some((d) => d.code === "save/migrated-to-idb"));
  check("legacy live key archived and removed", !legacyMap.has(STORAGE_KEY) && legacyMap.has(MIGRATED_KEY));
  ...
  check("save/load on real IndexedDB is state-identical",
  ...
  check("both records are present after commit",
    !!raw[coreKey("primary")] && !!raw[manifestKey("primary")]);

  await store.clear();
  check("clear wipes IndexedDB records and the legacy slot",
