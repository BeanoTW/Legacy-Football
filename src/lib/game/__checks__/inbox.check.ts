...
check(
  "fromAbsoluteWeek(48) = s2w2",
  JSON.stringify(fromAbsoluteWeek(48)) === '{"season":2,"week":2}',
);
check(
  "s1w44 + 4w = s2w2",
  fromAbsoluteWeek(absoluteWeek(1, 44) + 4).season === 2 &&
...
console.log("\n[6] Season-safe timing — cross-season follow-up");
{
  ...
  s = runWeeklyGenerators(s);
  check(
    "follow-up scheduled at s1w44+4 fires at s2w2",
...
console.log("\n[8] Previous-week finance lookup across rollover");
{
  ...
  check(
    "finance lookup finds s1w46 ledger from s2w1",
    !!fin,
