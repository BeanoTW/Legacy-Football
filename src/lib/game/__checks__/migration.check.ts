...
safe("missing version", () => {
  ...
  check(
    "missing version backfills saveSeed",
    typeof m.saveSeed === "string" && m.saveSeed.length > 0,
  );
});
safe("version 1", () => {
...
safe("version 2 idempotent", () => {
  ...
  delete srcAny.leagueSchedule;
  delete srcAny.matchRecords; // a genuine v2 save has neither
  const m = migrateSave(src);
  check(
    "v2 upgrades to v6 keeping seed",
    (m.version as number) === SAVE_VERSION && m.saveSeed === "KEEP_ME",
  );
  check(
    "v2 save gets empty league schedule (legacy season preserved)",
    Array.isArray(m.leagueSchedule) &&
      m.leagueSchedule.length === 0 &&
      Array.isArray(m.matchRecords) &&
      m.matchRecords.length === 0,
  );
  const again = migrateSave(m as unknown as Record<string, unknown>);
  check(
    "migration is idempotent",
    again.saveSeed === "KEEP_ME" && (again.version as number) === SAVE_VERSION,
  );
});
...
safe("null collections", () => {
  const m = migrateSave(
    legacy({ inbox: null, scheduledGenerators: null, inboxFlags: null, ledger: null }),
  );
  check(
    "null collections coerced to empty",
    Array.isArray(m.inbox) &&
      Array.isArray(m.scheduledGenerators) &&
      Array.isArray(m.ledger) &&
      typeof m.inboxFlags === "object",
  );
});
...
safe("expiresWeek → expiresAtAbsoluteWeek", () => {
  const m = migrateSave(
    legacy({
      inbox: [
        {
          id: "old-1",
          generatorId: "grounds-south-roof",
          sender: "Eddie",
  ...
  check(
    "expiresWeek → abs (s1w8 = 8)",
    it.expiresAtAbsoluteWeek === absoluteWeek(1, 8),
...
safe("dueWeek/dueSeason → dueAtAbsoluteWeek", () => {
  const m = migrateSave(
    legacy({
      scheduledGenerators: [
        { generatorId: "grounds-south-roof-followup", dueWeek: 2, dueSeason: 2 },
      ],
    }),
  );
  check(
    "s2w2 → abs 48",
    m.scheduledGenerators[0].dueAtAbsoluteWeek === absoluteWeek(2, 2),
...
safe("malformed scheduled entry is not silently dropped", () => {
  ...
  check(
    "recoverable entries retained with a due time",
  ...
  check(
    "entry with no generatorId is discarded",
...
safe("legacy cooldown flag", () => {
  ...
  check(
    "fansWarnedAtWeek → absolute using saved season",
...
safe("rollover save", () => {
  const m = migrateSave(
    legacy({
      ...
      week: 46,
      scheduledGenerators: [
        { generatorId: "grounds-south-roof-followup", dueWeek: 44, dueSeason: 1 },
      ],
    }),
  );
  const ticked = runWeeklyGenerators({ ...m, season: 2, week: 1 });
  check(
    "overdue follow-up fires after rollover instead of vanishing",
