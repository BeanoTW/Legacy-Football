...
check("every round has N/2 matches", schedule.every((r) => r.length === N / 2));

console.log("\n[2] Pairings meet exactly twice, once each way");
{
  const pairCount: Record<string, string[]> = {};
  for (const r of schedule) for (const m of r) {
    const k = [m.home, m.away].sort().join(" v ");
    (pairCount[k] ||= []).push(m.home);
  }
  const expected = (N * (N - 1)) / 2;
  check("all unique pairs present", Object.keys(pairCount).length === expected,
    `${Object.keys(pairCount).length}/${expected}`);
  check("each pair meets exactly twice", Object.values(pairCount).every((v) => v.length === 2));
  check("each pair has one home each", Object.values(pairCount).every((v) => v[0] !== v[1]));
...
check("no club twice in a round", schedule.every((r) => {
  const seen = new Set<string>();
  for (const m of r) { if (seen.has(m.home) || seen.has(m.away)) return false; seen.add(m.home); seen.add(m.away); }
  return seen.size === N;
}));

console.log("\n[4] Home/away totals + streaks");
const diag = scheduleDiagnostics(schedule, teams);
check("every club plays 19 home / 19 away",
...
for (const d of diag) {
  console.log(
    "  " + d.team.padEnd(22) +
    String(d.homeMatches).padEnd(4) + String(d.awayMatches).padEnd(4) +
    String(d.longestHomeStreak).padEnd(6) + String(d.longestAwayStreak),
  );
}
...
console.log("\n[6] Determinism");
{
  ...
  check("different seeds produce >1 distinct schedule", new Set(outs).size > 1,
    `${new Set(outs).size} distinct`);
  let allValid = true;
  for (const s of seeds) {
    const sch = buildSeasonSchedule(teams, s);
    const d = scheduleDiagnostics(sch, teams);
    if (!d.every((x) => x.homeMatches === 19 && x.awayMatches === 19
      && x.longestHomeStreak <= 2 && x.longestAwayStreak <= 2)) allValid = false;
  }
...
console.log("\n[7] User club is not special-cased");
{
  ...
  check("user club has same H/A totals as AI clubs",
    userDiag.homeMatches === 19 && userDiag.awayMatches === 19);
  check("user club streaks within same bounds as AI clubs",
  ...
  check("schedule independent of which club is user-controlled",
...
console.log("\n[8] Club fixture list / calendar mapping");
{
  ...
  check("weeks inside league windows", fx.every((f) =>
    (f.week >= CALENDAR.firstHalfStart && f.week <= CALENDAR.firstHalfEnd) ||
    (f.week >= CALENDAR.secondHalfStart && f.week <= CALENDAR.secondHalfEnd)));
  check("no mid-season-break fixtures", fx.every((f) =>
    f.week < CALENDAR.midSeasonStart || f.week > CALENDAR.midSeasonEnd));
  check("19 home / 19 away", fx.filter((f) => f.home).length === 19);
  check("first half is NOT all home",
    new Set(fx.filter((f) => f.week <= 23).map((f) => f.home)).size === 2);
  check("second half is NOT all away",
    new Set(fx.filter((f) => f.week >= 28).map((f) => f.home)).size === 2);
  check("each opponent faced exactly twice",
    Object.values(fx.reduce<Record<string, number>>((a, f) => {
      a[f.opponent] = (a[f.opponent] || 0) + 1; return a;
    }, {})).every((c) => c === 2));
  check("round->week mapping monotonic",
...
console.log("\n[9] New game + a newly generated second season");
{
  ...
  const s1 = g.fixtures;
  check("new game fixtures balanced across halves",
  ...
  check("season 2 streaks <= 2",
    d2.every((d) => d.longestHomeStreak <= 2 && d.longestAwayStreak <= 2));
  check("season 2 differs from season 1", JSON.stringify(s2) !== JSON.stringify(
    makeFixtures(CLUB, `${g.saveSeed}|season1`)));
}
