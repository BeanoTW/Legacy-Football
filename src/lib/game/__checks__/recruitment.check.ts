...
console.log("\n[R1] Player world");
{
  ...
  check(
    "2. same seed ⇒ byte-identical contracts",
    JSON.stringify(a.football.contracts) === JSON.stringify(b.football.contracts),
  );
  check(
    "3. different seed ⇒ materially different players",
  ...
  check(
    "5. identities survive a reload",
    JSON.stringify(reload(a).football.players) === JSON.stringify(a.football.players),
  );
  check(
    "6. every player has valid age/position/ability/potential/value",
    a.football.players.every((p) => {
  ...
  check(
    "7b. generateWorld is pure w.r.t. a throwaway state",
    (() => {
...
console.log("\n[R2] Ownership and squads");
{
  ...
  for (const c of s.football.contracts.filter(
    (x) => x.status === "Active" || x.status === "Expiring",
  )) {
    owners.set(c.playerId, [...(owners.get(c.playerId) ?? []), c.clubId]);
  }
  check(
    "8. every contracted player belongs to exactly one club",
    [...owners.values()].every((v) => v.length === 1),
  );
  const clubs = [
    ...new Set(s.football.players.map((p) => p.currentClubId).filter(Boolean)),
  ] as string[];
  const seen = new Set<string>();
  let dupe = false;
  for (const club of clubs)
    for (const p of squadOf(s, club)) {
      if (seen.has(p.id)) dupe = true;
      seen.add(p.id);
    }
  check("9. no player appears in two squads", !dupe);
  check(
    "10. squad membership matches active contracts",
    s.football.players.every((p) => {
      const c = activeContract(s, p.id);
      return p.currentClubId === null ? !c : !!c && c.clubId === p.currentClubId;
    }),
  );
  check(
    "11. currentClubId, contractId and contract agree",
    s.football.players.every((p) => {
      const c = activeContract(s, p.id);
  ...
  check(
    "12. free agents hold no active contract",
    freeAgents(s).every((p) => !activeContract(s, p.id)),
  );
  check(
    "13. squad generation is deterministic",
    JSON.stringify(userSquad(s)) === JSON.stringify(userSquad(fixture())),
  );
  check(
    "14. club squad sizes stay within bounds",
    clubs.every(
      (c) => squadOf(s, c).length >= MIN_SQUAD_SIZE && squadOf(s, c).length <= MAX_SQUAD_SIZE,
    ),
  );
  ...
  if (!n) {
    check("15/16. transfer completion moves the player", false, "no agreed deal reachable");
  } else {
    ...
    check(
      "16. completion adds the player to the buying squad exactly once",
...
console.log("\n[R3] Contracts");
{
  ...
  check(
    "17. at most one active contract per player",
    [...byPlayer.values()].every((v) => v === 1),
  );
  check(
    "18. active contracts have valid start/expiry",
    s.football.contracts.every(
      (c) => c.startSeason >= 1 && c.expirySeason >= c.startSeason && c.expiryWeek >= 1,
    ),
  );
  check(
    "19. contract expiry is absolute-week safe",
    s.football.contracts.every((c) => weeksLeftOnContract(s, c) > -1000),
  );

  const w = clone(s);
  const target = userSquad(w).find(
    (p) => renewalTerms(w, p.id) && renewContractInPlace(clone(w), p.id).ok,
  );
  if (!target) check("20-25. renewal", false, "no renewable player");
    ...
    check(
      "20. renewal supersedes the previous contract",
      r.ok && oldC.status !== "Active" && !!activeContract(w, target.id),
    );
    check(
      "21. renewal cannot create overlapping active contracts",
      w.football.contracts.filter(
        (c) => c.playerId === target.id && (c.status === "Active" || c.status === "Expiring"),
      ).length === 1,
    );
    check(
      "24. contract history is append-only",
      w.football.contractHistory.length === histBefore.length + 1 &&
        JSON.stringify(w.football.contractHistory.slice(0, histBefore.length)) ===
          JSON.stringify(histBefore),
    );
    const snapshot = clone(w.football.contractHistory);
    renewContractInPlace(w, target.id);
    check(
      "25. earlier history rows are immutable after later renewals",
      JSON.stringify(w.football.contractHistory.slice(0, snapshot.length)) ===
  ...
  check(
    "22. released players become free agents",
  ...
  check(
    "26. reload cannot duplicate expiry/renewal outcomes",
    JSON.stringify(replay.football.contractHistory) ===
      JSON.stringify(replay2.football.contractHistory),
  );
}
...
console.log("\n[R4] Transfer market");
{
  ...
  check(
    "28. availability always has a football reason",
  ...
  if (n) {
    ...
    check(
      "31/32/33. a completed negotiation cannot re-complete",
  ...
  if (nw) {
    ...
    check(
      "31. withdrawn negotiations cannot later complete",
      !completeTransferInPlace(wd, nw.id).ok,
    );
  ...
  check(
    "32. rejected negotiations cannot later complete",
  ...
  if (ne) {
    ...
    check(
      "33. expired negotiations cannot later complete",
  ...
  check(
    "34. transfer-window rules exist in the weekly runner",
    /runRecruitmentWeek\(\s*\w+\s*,\s*[^)]*window/i.test(
      readFileSync("src/lib/game/engine.ts", "utf8"),
    ),
  );
}
...
console.log("\n[R5] Club negotiation");
{
  ...
  const b = mk();
  check(
    "36. club negotiation outcomes are deterministic",
    JSON.stringify(a.n) === JSON.stringify(b.n),
  );
  check(
    "38. selling club uses canonical fee/contract data",
  ...
  check(
    "37. maximum club rounds are enforced",
  ...
  check(
    "39. accepted club terms create one negotiation only",
    !n2 || acc.football.negotiations.filter((x) => x.playerId === n2.playerId).length === 1,
  );
  check(
    "40. rejected club terms create no player talks",
    (() => {
  ...
  check(
    "41. counter-offers are stable after reload",
    (() => {
      const r = reload(a.s);
      return (
        JSON.stringify(negotiationById(r, a.n.id)) === JSON.stringify(negotiationById(a.s, a.n.id))
      );
    })(),
  );
  check(
    "42. reopening a negotiation cannot reroll its outcome",
    (() => {
      const r1 = reload(a.s);
      const r2 = reload(a.s);
  ...
  check(
    "43. duplicate offers for the same player are refused",
    (() => {
      const s = fixture("DUP");
      Object.assign(s, setTransferBudget(s, Math.min(20_000_000, Math.floor(s.cash))).state);
      const t = transferMarket(s).find((m) => m.clubId)!;
  ...
  check(
    "44. a completed club negotiation cannot reopen",
    (() => {
...
console.log("\n[R6] Player negotiation");
{
  const { improvePlayerTermsInPlace } =
    require("../recruitment") as typeof import("../recruitment");
  const build = () => {
  ...
  const b = build();
  check(
    "45. player negotiation outcomes are deterministic",
    JSON.stringify(a.n) === JSON.stringify(b.n),
  );
  check(
    "47. wage/length/bonus/role are evaluated canonically",
    a.n.proposedWeeklyWage === wageDemand(a.s, a.fa, a.n.proposedRole) &&
      a.n.proposedLengthSeasons >= 1 &&
      a.n.proposedRole.length > 0,
  );
  let guard = 0;
  while (negotiationById(a.s, a.n.id)!.stage === "playerTalks" && guard++ < 5) {
    ...
    if (
      !improvePlayerTermsInPlace(
        a.s,
        cur.id,
        (cur.playerCounterWage ?? cur.proposedWeeklyWage) + 500,
      ).ok
    )
      break;
  }
  check(
    "46. maximum player rounds are enforced",
    negotiationById(a.s, a.n.id)!.playerRounds <= MAX_NEGOTIATION_ROUNDS,
  );
  const fin = negotiationById(a.s, a.n.id)!;
  check(
    "48. accepted personal terms create exactly one agreed deal",
    fin.stage !== "agreed" ||
      a.s.football.negotiations.filter((n) => n.playerId === fin.playerId && n.stage === "agreed")
        .length === 1,
  );
  check(
    "49. rejected personal terms cannot complete",
    fin.stage !== "rejected" || !completeTransferInPlace(a.s, fin.id).ok,
  );
  check(
    "50. countered terms remain stable after reload",
    JSON.stringify(negotiationById(reload(a.s), a.n.id)) === JSON.stringify(fin),
  );
  check(
    "51. a player cannot hold two open negotiations",
    (() => {
      const counts = new Map<string, number>();
      for (const n of openNegotiations(a.s))
        counts.set(n.playerId, (counts.get(n.playerId) ?? 0) + 1);
...
console.log("\n[R7] Transfer completion");
{
  ...
  if (!n) {
    ...
    check(
      "55. the selling contract closes exactly once",
      s.football.contractHistory.filter(
        (h) => h.playerId === n.playerId && h.outcome === "transferred",
      ).length === (hadSeller ? 1 : 0),
    );
    check(
    ...
    check(
      "57. transfer history appends exactly once",
      s.football.transferHistory.length === historyBefore + 1,
    );
    check(
      "58. contract history appends correctly",
    ...
    check(
      "60. the signing bonus posts exactly once",
    ...
    check(
      ...
      (() => {
        ...
        const snap = JSON.stringify({
          c: g.cash,
          p: g.football.players,
          k: g.football.contracts,
          f: g.financeLedger.length,
        });
    ...
    check(
      "65. reloading after completion cannot duplicate the deal",
      (() => {
        const g = reload(s);
        const res = completeTransferInPlace(g, n.id);
    ...
    check(
      "66. one agreement can only move a player once",
...
console.log("\n[R8] Incoming bids and sales");
{
  ...
  if (!found || !found2) {
    ...
    check(
      "67. incoming bids generate deterministically",
      JSON.stringify(found.s.football.negotiations) ===
        JSON.stringify(found2.s.football.negotiations),
    );
    ...
    check(
      "69. accepting a bid moves it to one completion path",
    ...
    check(
      "72. sale proceeds post exactly once",
      s.financeLedger.filter((e) => e.dedupeKey === `transfer:${n.id}:sale`).length ===
        (n.fee > 0 ? 1 : 0),
    );
    check(
      "72b. cash rises by the fee and reconciles",
      s.cash === cashBefore + n.fee && reconciles(s),
    );
    check(
      "73. the sold player leaves the squad exactly once",
    ...
    check(
      ...
      !!rec &&
        ...
        rec.fee === n.fee &&
        rec.season === s.season &&
        typeof rec.absoluteWeek === "number",
    );
    check(
      "75. an accepted bid cannot be accepted again",
    ...
    check(
      "71. countering an incoming bid is deterministic",
      JSON.stringify(negotiationById(c1.s, c1.n.id)) ===
        JSON.stringify(negotiationById(c2.s, c2.n.id)),
    );
  }
}
...
console.log("\n[R9] Wages and finance");
{
  ...
  check(
    ...
    userWageBill(s) === Math.round(fromContracts),
  );

  let g = s;
  for (let i = 0; i < 10; i++) g = advanceWeek(g);
  const wageKeys = g.financeLedger
    .filter((e) => e.subcategory === "Player wages")
    .map((e) => e.dedupeKey);
  check(
    "77. weekly wage posting happens once per eligible week",
    new Set(wageKeys).size === wageKeys.length && wageKeys.length > 0,
  );
  const r1 = advanceWeek(reload(g));
  const r2 = advanceWeek(reload(g));
  check(
    "78. reloading cannot duplicate wage payments",
    r1.financeLedger.length === r2.financeLedger.length &&
      JSON.stringify(r1.financeLedger.map((e) => e.dedupeKey)) ===
        JSON.stringify(r2.financeLedger.map((e) => e.dedupeKey)),
  );

  let season2 = g;
  while (season2.season < 2) season2 = advanceWeek(season2);
  const allKeys = season2.financeLedger
    .filter((e) => e.subcategory === "Player wages")
    .map((e) => e.dedupeKey);
  check(
    "79. season rollover neither skips nor duplicates wages",
  ...
  check(
    "80. released players stop generating wages immediately",
  ...
  check(
    "85. no direct cash mutation in recruitment code",
    !/\bs\.cash\s*[-+]?=/.test(src) && !/\bw\.cash\s*[-+]?=/.test(src),
  );
  check(
    "86. no hand-built ledger rows in recruitment code",
    !/finance\.entries\.push|ledger\.push/.test(src),
  );
  check(
    "87. finance still reconciles after buys, sales, renewals and releases",
...
console.log("\n[R10] Board integration");
{
  ...
  check(
    "89. net-spend objective exists and reads canonical records",
    kinds.includes("transferBudgetDiscipline") &&
      netSpendThisSeason(s) === netSpendThisSeason(reload(s)),
  );
  check("90. sale-income objective exists", kinds.includes("playerSaleIncome"));
  check(
    "91. squad-age objective reads canonical squad membership",
    kinds.includes("squadAge") && Math.abs(averageSquadAge(s) - averageSquadAge(reload(s))) < 1e-9,
  );
  check(
    "92. contract-security objective reads active contract expiries",
    kinds.includes("contractSecurity") && contractSecurityPct(s) === contractSecurityPct(reload(s)),
  );
  check(
    "93. signings-completed objective reads completed transfers",
...
console.log("\n[R11] Inbox integration");
{
  ...
  check(
    "97-101. recruitment messages generate at most once each",
    new Set(keys).size === keys.length,
  );
  check(
    "102. recruitment event keys are stable",
  ...
  check(
    "103. inbox actions call the canonical recruitment engine",
    /InPlace\(/.test(inboxSrc) && /from "\.\/recruitment"/.test(inboxSrc),
  );
  check(
    "105. informational completion messages carry no money effects",
    (() => {
      const info = items.filter((i) => i.eventKey.startsWith("recruitment-transfer-complete"));
      return info.every((i) =>
        (i.choices ?? []).every((c) => (c.effects ?? []).every((e) => e.kind !== "cash")),
      );
    })(),
  );
  ...
  if (bidItem && bidItem.choices?.length) {
    ...
    const first = applyEffects(g, ch.effects ?? [], {
      sourceItemId: bidItem.id,
      sourceEventKey: bidItem.eventKey,
    });
    const second = applyEffects(first, ch.effects ?? [], {
      sourceItemId: bidItem.id,
      sourceEventKey: bidItem.eventKey,
    });
    check(
      "104. reopening a message cannot repeat recruitment effects",
      JSON.stringify(first.football.negotiations) ===
        JSON.stringify(second.football.negotiations) && first.cash === second.cash,
    );
    check(
      "106. an expired decision cannot still complete a deal",
    ...
    check(
      "104/106. recruitment inbox effects are idempotent",
      false,
      "no incoming-offer message present",
    );
  }
}
...
console.log("\n[R12] UI and selectors");
{
  ...
  const routeSrc =
    ...
    readdirSync("src/components/game")
      ...
      .join("\n");
  check(
    "114. old Squad and Transfers interfaces are unreachable",
    !/function Squad\(|function Transfers\(|function TargetCard\(|function BidCard\(/.test(
      routeSrc,
    ),
  );
  check(
    "114b. no legacy transfer engine paths remain",
  ...
  check(
    "115. migrated saves render safely in the Recruitment tab",
    (() => {
...
console.log("\n[R13] History");
{
  ...
  for (let i = 0; i < 10; i++) g = advanceWeek(g);
  check(
    "116. transfer history is immutable",
    JSON.stringify(g.football.transferHistory.slice(0, s.football.transferHistory.length)) ===
      tSnap,
  );
  check(
    "117. contract history is immutable",
    JSON.stringify(g.football.contractHistory.slice(0, s.football.contractHistory.length)) ===
      cSnap,
  );
  check(
    "118. recruitment season summaries are immutable",
  ...
  check(
    "120. historical wage totals reconcile to contract data",
    userWageBill(g) ===
  ...
  check(
    "121. later moves do not rewrite earlier records",
    g.football.transferHistory
      .slice(0, 5)
      .every((r, i) => JSON.stringify(r) === JSON.stringify(s.football.transferHistory[i])),
  );
  check(
    "121b. ids in the transfer record are unique",
...
console.log("\n[R14] Migration");
{
  ...
  check(
    "124. finance data is preserved",
    m1.financeLedger.length === src.financeLedger.length && reconciles(m1),
  );
  check("125. board data is preserved", m1.board.directors.length === src.board.directors.length);
  check(
    "126. commercial data is preserved",
  ...
  check("129. legacy transfers do not invent history", m1.football.transferHistory.length === 0);
  check(
    "129b. retired legacy transfer collections are dropped",
...
console.log("\n[R15] Static audit");
{
  ...
  check(
    "S1. no Math.random()/Date.now() in recruitment/finance/inbox simulation",
    [
      "src/lib/game/recruitment.ts",
      "src/lib/game/finance.ts",
      "src/lib/game/inbox.ts",
      "src/lib/game/commercial.ts",
      "src/lib/game/fixtures.ts",
    ].every((f) => !/Math\.random\(/.test(codeOf(f)) && !/Date\.now\(/.test(codeOf(f))),
  );
  check(
    "S2. no direct cash writes outside finance.ts",
  ...
  check(
    "S3. no direct transfer-budget writes outside engine/budgets/finance/migrations",
    offenders(/\.transferBudget\s*[-+*]?=\s/).filter(
      (f) => !/engine\.ts|budgets\.ts|finance\.ts|migrations\//.test(f.replace(/\\/g, "/")),
    ).length === 0,
  );

  check(
    "S4. no squad or contract mutation in components/routes",
    offenders(/football\.(contracts|players|negotiations)\.(push|splice)/).filter((f) =>
      /components|routes/.test(f),
    ).length === 0,
  );
  check(
    "S5. legacy transfer types are gone from live state",
