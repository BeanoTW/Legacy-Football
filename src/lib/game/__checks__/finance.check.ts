...
console.log("\n[P1] Cash mutation audit (static)");
{
  ...
  check("no cash mutation outside finance.ts postEntry path", offenders.length === 0, offenders.join(" | "));

  const financeSrc = readFileSync("src/lib/game/finance.ts", "utf8");
  const bodies = financeSrc.split("\n").filter((l) => /s\.cash\s*=/.test(l));
  check("finance.ts writes cash in a small number of places", bodies.length <= 4, `${bodies.length} sites: ${bodies.map((b) => b.trim()).join(" | ")}`);
}
...
safe("season simulation", () => {
  ...
  check("cash reconciles after every one of 46 weeks", allWeeksReconcile, JSON.stringify(reconcile(s)));
  check("season rollover reconciles", reconciles(s));
  check("entries were actually produced", entriesFor(s, 1).length > 50, `${entriesFor(s, 1).length} entries`);
  ...
  check("dedupe keys unique", new Set(keys).size === keys.length, `${keys.length - new Set(keys).size} duplicates`);

  let projOk = true;
  const detail: string[] = [];
  for (const row of s.ledger) {
    const es = entriesFor(s, row.season, row.week).filter((entry) => entry.sourceSystem !== "engine.opening");
  ...
  check("weekly ledger is a faithful projection of operating entries", projOk, detail.slice(0, 3).join(" | "));

  const lastRow = s.ledger[s.ledger.length - 1];
  const lastEntry = s.financeLedger[s.financeLedger.length - 1];
  check("closing balance of last row equals recorded balanceAfter", lastRow.balance === lastEntry.balanceAfter, `${lastRow.balance} vs ${lastEntry.balanceAfter}`);

  const t = seasonTotals(s, 1);
  const allSeasonEntries = entriesFor(s, 1);
  const es1 = allSeasonEntries.filter((entry) => entry.sourceSystem !== "engine.opening");
  check(
  ...
  const openingIncome = allSeasonEntries.filter((e) => e.sourceSystem === "engine.opening" && e.direction === "income").reduce((a, e) => a + e.amount, 0);
  const grossIncomeIncludingOpening = allSeasonEntries.filter((e) => e.direction === "income").reduce((a, e) => a + e.amount, 0);
  check("opening capital is not reported as season income", t.income === grossIncomeIncludingOpening - openingIncome);

  check(
    "prize money awarded exactly once",
    s.financeLedger.filter((e) => e.category === "Prize Money").length <= 1 ||
      new Set(s.financeLedger.filter((e) => e.category === "Prize Money").map((e) => e.dedupeKey)).size === s.financeLedger.filter((e) => e.category === "Prize Money").length,
  );
...
safe("recurring week replay", () => {
  ...
  check("re-posting adds no entries", s.financeLedger.length === n, `${s.financeLedger.length} vs ${n}`);
...
safe("reload then continue", () => {
  ...
  check("reload does not change entry count", reloaded.financeLedger.length === s.financeLedger.length);
...
function spendCheck(
  ...
  check(`${label}: cash moved by exactly the cost`, s1.cash === before + (bucket === "expense" ? -expectCost : expectCost), `${s1.cash} vs ${before - expectCost}`);
  check(`${label}: exactly one new entry`, s1.financeLedger.length === n + 1, `${s1.financeLedger.length - n} entries`);
...
safe("capital project spends only through the ledger", () => {
  ...
  check("instalments are booked as facilities expenses", ticked.financeLedger.some((e) => e.sourceSystem === "facilities" && e.direction === "expense" && e.linkedEntityId === pid));
  check("state still reconciles after the instalments", reconciles(ticked), JSON.stringify(reconcile(ticked)));
...
safe("legacy transfer budget control cannot ring-fence cash", () => {
  ...
  const cash = s.cash;
  const startingTransferBudget = s.transferBudget;
  ...
  check("input state remains untouched", s.cash === cash && s.transferBudget === startingTransferBudget);
...
safe("inbox effects post to the ledger", () => {
  ...
  check("two new finance entries", s1.financeLedger.length === n + 2, `${s1.financeLedger.length - n}`);
  check("inbox spend reconciles", reconciles(s1));
  check("inbox entries tagged to source", s1.financeLedger.slice(-2).every((e) => e.linkedEntityId === "item-1" && e.sourceSystem === "inbox"));
  const row = s1.ledger.find((l) => l.season === s1.season && l.week === s1.week)!;
  check("legacy buckets honoured in projection", row.expenses.maintenance === 12_500 && row.income.sponsor === 40_000, JSON.stringify({ m: row.expenses.maintenance, sp: row.income.sponsor }));
...
safe("v6 ledger-only save", () => {
  ...
  check("legacy buckets round-trip", m.financeLedger.some((e) => legacyIncomeBucket(e) === "gate") && m.financeLedger.some((e) => legacyExpenseBucket(e) === "playerWages"));
  ...
  check("migration idempotent on entries", again.financeLedger.length === m.financeLedger.length, `${again.financeLedger.length} vs ${m.financeLedger.length}`);
