...
import {
  ...
  operatingPicture,
  structuralWeeklyExpenditure,
  structuralWeeklyIncome,
  capitalCommitments,
  capitalCommitmentsWithin,
  committedWages,
  contractExposure,
  ...
  sustainabilityConfidenceAdjustment,
  ...
  openCommitments,
  commitmentProgress,
  settleCommitmentsInPlace,
  runSustainabilityWeek,
...
console.log("\n[A] Purity");
{
  ...
  check(
    "A2. selectors are deterministic for one state",
  ...
  check(
    "A4. selectors post no ledger entries",
    MID.financeLedger.length === (JSON.parse(before) as GameState).financeLedger.length,
  );
  check(
    "A5. snapshot exposes every headline picture",
    (() => {
...
console.log("\n[B] Recommended reserve");
{
  ...
  check(
    "B3. excess = max(0, cash − recommended)",
    r.excess === Math.max(0, r.cash - r.recommended),
  );
  check(
    "B4. deficit = max(0, recommended − cash)",
  ...
  check(
    "B9. a heavier wage bill raises the recommended reserve",
  ...
  check(
    "B10. capital commitments within 26 weeks are added to the reserve",
    recommendedReserve(capital) >= recommendedReserve(MID),
  );
  check(
    "B11. near-term commitments never exceed total commitments",
    capitalCommitmentsWithin(MID, 26) <= capitalCommitments(MID) + 1,
  );
  check(
    "B12. reserve is a pure function of state",
...
console.log("\n[C] Needs");
{
  ...
  check(
    "C3. overall need sits between the best and worst area",
  ...
  check(
    "C4. rotting facilities raise infrastructure need",
  ...
  check("C5. unhappy supporters raise supporter need", supporterNeed(unhappy) > supporterNeed(MID));
  ...
  check("C6. an empty squad raises squad need", squadNeed(gutted) >= squadNeed(MID));
  check("C7. commercial need stays in range for a normal club", inRange(commercialNeed(MID)));
}
...
console.log("\n[D] Reinvestment pressure");
{
  ...
  check("D2. means and need are 0..1", p.means >= 0 && p.means <= 1 && p.need >= 0 && p.need <= 1);
  check(
    "D3. every area score is 0..100",
  ...
  check(
    "D5. cash alone does not create pressure",
    rh.score < rn.score,
    `${rh.score} vs ${rn.score}`,
  );
...
console.log("\n[E] Director stances");
{
  ...
  for (const a of rich.infrastructure?.assets ?? []) {
    a.condition = 6;
    a.qualityRating = 6;
  }
  ...
  check(
    "E1. every stance is -100..100",
    boardStances(rich).every((x) => x.stance >= -100 && x.stance <= 100),
  );
  check(
    "E2. every stance carries a note",
    boardStances(rich).every((x) => x.note.length > 5),
  );
  check(
    "E3. Finance Director resists spending when the reserve is short",
    st(broke, "Finance Director").stance < 0,
    String(st(broke, "Finance Director").stance),
  );
  check(
    "E4. Finance Director softens once cover is deep",
    st(rich, "Finance Director").stance > st(broke, "Finance Director").stance,
  );
  check(
    "E5. Supporters' Director reacts to visible neglect + reserves",
    st(rich, "Supporters' Director").stance > st(MID, "Supporters' Director").stance,
  );
  check(
    "E6. Supporters' Director wants supporter spend",
    st(rich, "Supporters' Director").wants === "supporters",
  );
  check(
    "E7. Commercial Director wants commercial spend",
    st(rich, "Commercial Director").wants === "commercial",
  );
  check(
    "E8. Football Director tracks squad need",
...
console.log("\n[F] Commitments");
{
  ...
  check(
    "F9. an expired, undelivered promise fails once",
    outcomes.length === 1 && outcomes[0].outcome === "failed",
  );
  check(
    "F10. failure costs the owning director's confidence",
  ...
  check(
    "F12. settlement is exactly-once",
  ...
  check(
    "F14. the outcome is recorded in history",
  ...
  check(
    "F15. a promise with no investment target cannot be fulfilled by doing nothing",
    z!.status === "failed",
  );
  ...
  check(
    "F16. a repeated broken promise costs more confidence",
  ...
  check(
    "F18. commitments survive a save round-trip",
    (() => {
      const back = migrateSave(
        JSON.parse(JSON.stringify(failing)) as unknown as Record<string, unknown>,
      );
...
console.log("\n[G] Capacity and wage pressure");
{
  ...
  check(
    "G9. staff cost ratio is reported and bounded",
    staffCostRatio(MID) >= 0 && staffCostRatio(MID) <= 300,
  );
  check("G10. infrastructure cost ratio is reported", infrastructureCostRatio(MID) >= 0);
  check("G11. committed wages are forward-looking", committedWages(MID) > 0);
  check(
    "G12. contract exposure is a percentage",
    contractExposure(MID) >= 0 && contractExposure(MID) <= 100,
  );
  check(
    "G13. commercial concentration is a percentage",
...
console.log("\n[H] Financial health");
{
  ...
  check(
    "H3. trajectory is one of three values",
  ...
  check(
    "H4. a club with no money is not rated healthy",
    bh.state === "critical" || bh.state === "stressed",
    bh.state,
  );
  check(
    "H5. insolvency risk fires only when critical AND overdrawn",
  ...
  check(
    "H7. deep cover improves the rating",
    STATES.indexOf(financialHealth(rich).state) <= STATES.indexOf(h.state),
  );
  check(
    "H8. health is derived only",
    (() => {
      const before = sig(MID);
      financialHealth(MID);
      return sig(MID) === before;
    })(),
  );
}
...
console.log("\n[I] Weekly tick");
{
  ...
  check("I4. the tick stamps the absolute week", (s.sustainability?.lastTickAbsoluteWeek ?? 0) > 0);
  ...
  check(
    "I6. peak idle weeks are tracked",
  ...
  check(
    "I12. migrated state matches the default shape",
    Object.keys(m.sustainability!).sort().join(",") ===
      Object.keys(defaultSustainability()).sort().join(","),
  );
  check(
    "I13. migration is idempotent",
    JSON.stringify(
      migrateSave(JSON.parse(JSON.stringify(m)) as unknown as Record<string, unknown>)
        .sustainability,
    ) === JSON.stringify(m.sustainability),
  );
  check(
    "I14. ensureSustainability is safe to call repeatedly",
    (() => {
      const e = clone(MID);
      ensureSustainability(e);
      const a = sig(e);
      ensureSustainability(e);
      return sig(e) === a;
    })(),
...
console.log("\n[J] Passive economy audit");
{
  ...
  for (let season = 1; season <= 10; season++) {
    ...
    rows.push({
      ...
      reserve: recommendedReserve(s),
      pressure: p.score,
      need: Math.round(p.need * 100) / 100,
  ...
  for (const r of rows) {
    console.log(
      `    S${String(r.season).padStart(2)}  cash=${r.cash.toLocaleString().padStart(12)}  reserve=${r.reserve.toLocaleString().padStart(11)}  pressure=${String(r.pressure).padStart(3)}  need=${r.need}  health=${r.health}  idle=${r.idleWeeks}w`,
    );
  }
  ...
  check(
    "J8. sustainability state stays internally consistent",
    (s.sustainability?.peakExcessWeeks ?? 0) >= (s.sustainability?.excessWeeks ?? 0),
  );
  check(
    "J9. no commitment is invented by passive play",
    (s.sustainability?.commitments ?? []).length === 0,
  );
  check(
    "J10. a trailing operating picture is available at every mark",
...
console.log("\n[L] Integration wiring");
{
  ...
  check(
    "L2. idle capital changes at least one director's stance",
    roles.some(
      (r) =>
        sustainabilityConfidenceAdjustment(rich, r) !== sustainabilityConfidenceAdjustment(s, r),
    ) || anyNonZero,
  );
  ...
  check(
    "L3. confidence stays bounded after a review",
