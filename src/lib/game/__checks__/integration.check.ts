...
console.log("\n[I3] Pre-season friendlies are seeded, not random");
{
  ...
  const base = fresh("FRIENDLY_SEED");
  const a = advanceWeek(clone(base));
  const b = advanceWeek(clone(base));
  check("same state ⇒ same friendly result",
    JSON.stringify(a.results) === JSON.stringify(b.results));
  const other = { ...clone(base), saveSeed: "OTHER_SEED" } as GameState;
  const c = advanceWeek(other);
  check("a different save seed can change the friendly",
    JSON.stringify(c.results) !== JSON.stringify(a.results) || a.results.length === 0);
  check("friendly booked matchday finance",
    a.financeLedger.length >= base.financeLedger.length);
}
