...
console.log("\n[B1] Board composition");
{
  ...
  check(
    "chairman is present",
    b.directors.some((d) => d.role === "Chairman"),
  );
  check(
    "every director has 2 traits",
    b.directors.every((d) => d.traits.length === 2),
  );
  check(
    "no director is both patient and ruthless",
  ...
  check(
    "influence sums to a sane boardroom",
    (() => {
  ...
  check(
    "chairman holds the largest influence",
    b.directors.every(
      (d) =>
        d.role === "Chairman" ||
        d.influence <= b.directors.find((x) => x.role === "Chairman")!.influence,
    ),
  );
}
...
console.log("\n[B3] Objectives");
{
  ...
  check(
    "objectives generated for season 1",
    objs.length === 12 && objs.every((o) => o.season === 1),
  );
  check("all objective ids unique", new Set(objs.map((o) => o.id)).size === objs.length);
  check(
    "every objective starts active",
    objs.every((o) => o.status === "active"),
  );
  check(
  ...
  check(
    "every priority area is owned by a seated director",
...
console.log("\n[B4] Objective evaluation is bounded and live");
{
  ...
  check(
    "huge cash fully satisfies the reserve objective",
    evaluateObjective(rich, cashObj).progress === 1,
  );
  const broke = { ...s, cash: 0 } as GameState;
  check(
    "zero cash fails the reserve objective",
...
console.log("\n[B5] Directors disagree");
{
  ...
  check(
    "finance director happier than supporters' director when cash-rich and fan-poor",
...
console.log("\n[B6] Confidence aggregation");
{
  ...
  check(
    "bands map correctly",
    confidenceBand(90) === "secure" &&
      confidenceBand(70) === "stable" &&
      confidenceBand(50) === "watchful" &&
      confidenceBand(30) === "strained" &&
...
console.log("\n[B8] Full season rollover");
{
  ...
  check(
    "season 1 objectives were graded",
    end[0] ? end[0].outcomes.every((o) => typeof o.met === "boolean") : false,
  );
  check(
    "new objectives issued for season 2",
  ...
  check("headline confidence stays in range", s.board.confidence >= 0 && s.board.confidence <= 100);
}
...
console.log("\n[B9] Review idempotency");
{
  ...
  s = advanceWeek(s);
  check(
    "no duplicate review added the following week",
    s.board.reviews.filter((r) => r.season === 1 && r.type === "midSeason").length === 1,
  );
  check(
    "confidence unchanged between reviews",
    s.board.confidence === conf,
...
console.log("\n[B10] Confidence floors respected");
{
  ...
  for (const d of s.board.directors) {
    ...
    check(
      `${d.role} respects confidence floor`,
      d.confidence >= floor,
      `${d.confidence} < ${floor}`,
    );
  }
...
console.log("\n[B11] Migration from a boardless save");
{
  ...
  check(
    "migrated save has current-season objectives",
