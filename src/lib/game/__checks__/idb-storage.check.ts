...
console.log("\n[D2] Legacy localStorage migration");
{
  ...
  check(
    "7-8. legacy save is detected and migrated",
    res.state !== null && res.diagnostics.some((d) => d.code === "save/migrated-to-idb"),
  );
  check(
    "9. migrated state equals the source after game migration",
    stateHash(res.state!) === stateHash(s),
  );
  check(
    "10. legacy live key removed only after a verified commit",
    !legacy.map.has(STORAGE_KEY) &&
      legacy.map.has(MIGRATED_KEY) &&
      !!(await records.get([K.core]))[K.core],
  );

  const again = await store.load();
  check(
    "12. re-running load does not duplicate state",
    stateHash(again.state!) === stateHash(s) &&
      (await records.keys()).length === 2 &&
...
}
{
  ...
  check(
    "    failed migration is reported",
    res.diagnostics.some((d) => d.code === "save/legacy-migration-failed"),
  );
...
}
{
  ...
  const res = await store.load();
  check(
    "corrupt legacy save leaves the source untouched",
    legacy.map.get(STORAGE_KEY) === "{not json",
  );
  check(
    "corrupt legacy save is preserved and reported",
    res.state === null && res.diagnostics.some((d) => d.code === "save/parse-failed"),
  );
  const blocked = await store.save(fresh());
  check(
    "writes stay blocked while the legacy save is unreadable",
...
console.log("\n[D3] Future-version protection");
{
  ...
  const res = await store.load();
  check(
    "13. future-version save is rejected safely",
    res.state === null &&
  ...
  check(
    "14. future-version source remains untouched",
    (await records.get([K.core]))[K.core] === core,
  );
  const diags = await store.save(fresh());
  check(
    "15. no new game overwrites it automatically",
...
}
{
  ...
  await records.putAll([
    { key: K.core, value: core },
    {
      ...
      value: JSON.stringify({
        ...
        gameSchemaVersion: SAVE_VERSION,
        saveSeed: SEED,
        controlledClubId: "IDB City",
        createdAt: 1,
        updatedAt: 1,
  ...
  const res = await store.load();
  check(
    "future STORAGE format is rejected separately from game schema",
    res.state === null && res.diagnostics.some((d) => d.code === "save/future-storage-format"),
  );
  check(
    "future storage-format save is not overwritten",
...
console.log("\n[D4] Atomicity + integrity");
{
  ...
  check(
    "16. failed transaction does not expose a partial save",
    diags.some((d) => d.code === "save/write-failed"),
  );
  records.options.failWrites = false;
  const after = await store.load();
  check(
    "17. previous valid save remains readable after a failed overwrite",
...
}
{
  ...
  const res = await store.load();
  check(
    "18. missing core record is detected",
...
}
{
  ...
  const res = await store.load();
  check(
    "19. checksum/manifest mismatch is detected",
    res.state === null && res.diagnostics.some((d) => d.code === "save/checksum-mismatch"),
  );
  check(
    "    a mismatched save is preserved, not overwritten",
    (await store.save(fresh())).some((d) => d.code === "save/write-blocked"),
  );
}
{
  ...
  check(
    ...
    res.state === null && res.diagnostics.some((d) => d.code === "save/manifest-invalid"),
  );
}
...
console.log("\n[D5] Metadata separation");
{
  ...
  check(
    "20b. loaded state hash is unaffected by storage metadata",
    stateHash(loaded) === stateHash(s),
  );
  ...
  check(
    "21. manifest data is not injected into GameState",
    manifestOnly.every((k) => !stateKeys.includes(k)),
  );

  const m = await a.readManifest();
  check(
    "22. storage format version is separate from the game schema version",
    isManifest(m) &&
      m!.storageFormatVersion === STORAGE_FORMAT_VERSION &&
      m!.gameSchemaVersion === SAVE_VERSION &&
      (STORAGE_FORMAT_VERSION as number) !== (SAVE_VERSION as number),
  );
  check(
    "    manifest carries save identity for future multi-slot use",
...
console.log("\n[D6] Clear/reset across every state");
{
  ...
  await store.clear();
  check(
    "reset after a migrated legacy save wipes both stores",
...
}
{
  ...
  await store.clear();
  check(
    "reset after a future-version save unblocks writing",
...
}
{
  ...
  check(
    "reset after a failed migration allows a new save",
    (await store.save(fresh())).length === 0,
  );
}
...
console.log("\n[D7] Scale + benchmarks");
{
  ...
  check(
    "25. a season-5 save beyond the old localStorage danger line stores and loads",
    diags.length === 0 && loaded.state !== null && rawBytes > 4_000_000,
  );
  check(
    "25b. reload is the deterministic compact hot core",
    loaded.state !== null && stateHash(loaded.state!) === stateHash(expectedCompact),
  );
  check(
    "25c. historical detail is split into chunk records",
    isManifest(manifest) && chunkCount > 0,
  );
  check(
    "25d. compaction materially shrinks the persisted core",
  ...
  check(
    "23. load stays within the development baseline (<2000 ms)",
    loadMs < 2000,
    `${loadMs.toFixed(1)} ms`,
  );
  check(
    "24. save stays within the development baseline (<2000 ms)",
    saveMs < 2000,
    `${saveMs.toFixed(1)} ms`,
  );
  check(
    "metrics account for core plus chunk records",
    isManifest(manifest) &&
      store.metrics.lastSaveMs !== null &&
...
console.log("\n[D8] Static audit: persistence stays inside the storage module");
{
  ...
  walk("src");
  check(
    "no persistence details outside lib/game/storage",
    offenders.length === 0,
    offenders.join(", "),
  );
  ...
  check(
    "engine talks only to the SaveStore factory",
  ...
  check(
    "no storage metadata leaked into GameState types",
