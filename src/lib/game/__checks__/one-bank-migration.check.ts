...
console.log("\n[OBM1] v13 ring-fenced transfer funds are released exactly once");
{
  ...
  assert(migrated.cash === beforeCash + 125_000, "ring-fenced funds return to the club bank balance once");
  const releases = migrated.financeLedger.filter((entry) => entry.dedupeKey === KEY);
  assert(releases.length === 1, "migration writes exactly one canonical budget-release ledger entry");
  assert(releases[0].amount === 125_000 && releases[0].direction === "income", "release entry records the exact retired pot");
  assert(
    JSON.stringify({
  ...
  assert(remigrated.cash === migrated.cash, "re-migrating a current save cannot release the funds again");
  assert(remigrated.transferBudget === 0, "re-migration keeps the legacy pot retired");
  assert(remigrated.financeLedger.filter((entry) => entry.dedupeKey === KEY).length === 1, "re-migration cannot duplicate the release ledger entry");
}
...
console.log("\n[OBM2] v13 saves with no transfer pot fabricate no release");
{
  ...
  assert(!migrated.financeLedger.some((entry) => entry.dedupeKey === KEY), "zero-pot migration writes no fake budget-release entry");
}

console.log("\n[OBM3] v14+ saves never execute the v13 cash-release step");
{
  const current = newGame("Migration City", "Ada Migration", "ONE|BANK|V14");
  const v14 = structuredClone(current) as GameState;
  v14.version = 14;
  v14.transferBudget = 0;
  const beforeCash = v14.cash;
  const beforeEntries = v14.financeLedger.length;
  ...
  assert(migrated.financeLedger.length === beforeEntries, "v14 migration adds no transfer-release ledger movement");
}
