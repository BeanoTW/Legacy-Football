 ...
 *
 * The historical chain is frozen through v12. Newer migrations are invoked
 * through their production steps after that boundary so parity keeps testing
 * the old implementation without duplicating newer migration bodies.
 */
...
import { V13_TO_V14 } from "../migrations/v13-v14";
import { V14_TO_V15 } from "../migrations/v14-v15";
...
export function legacyMigrateSave(parsed: Record<string, unknown>): GameState {
  ...
  if (p.version < 2) {
    ...
    p.scheduledGenerators = p.scheduledGenerators
      ...
      .map((g) => {
        if (typeof g.dueAtAbsoluteWeek === "number" && Number.isFinite(g.dueAtAbsoluteWeek)) return g;
  ...
  const ctx = { deps: { staffPoolFor, squadRating }, warn: () => undefined };
  if (p.version < 13 && SAVE_VERSION >= 13) {
    V12_TO_V13.up(p as never, ctx);
    p.version = 13;
  }
  if (p.version < 14 && SAVE_VERSION >= 14) {
    V13_TO_V14.up(p as never, ctx);
    p.version = 14;
  }
  if (p.version < 15 && SAVE_VERSION >= 15) {
    V14_TO_V15.up(p as never, ctx);
    p.version = 15;
  }
